...
## 2026-06-04 (suite) — Phase 06.17 cadrée + exécutée

Phase 06.17 « Qualité de complétion des formulaires » cadrée + livrée dans une seule PR. Application directe des normes UX/a11y (NN/G, Deque, Shopify Polaris, USWDS, W3C APG) — pas de débat d'architecture. Composant `<Field>` commun avec `hint` persistant lié par `aria-describedby` (D-01/D-02). Champs numériques bornés passés en `type=text` + `inputMode=numeric` + `pattern=[0-9]*` (D-03 — élimine les spinners et la capture molette). Composant `<Combobox>` éditable 100 % maison conforme W3C ARIA APG (role/aria/clavier/filtrage flou/saisie libre par défaut) — DEC-003 préservée (0 nouvelle dépendance npm, sans cmdk ni Radix Popover). Catalogue `lib/vehicles/catalog.ts` (Renault/Peugeot/Citroën/Dacia/Mercedes/VW/Ford/Toyota/Opel/Fiat/Nissan/Hyundai/Kia, ~70 modèles). Marque/Modèle = comboboxes dépendantes sur le formulaire véhicule. Normalisation Title Case au submit côté Server Action (D-07). `driver-form` migré sur `<Field>` commun (fin des Field locaux dupliqués). 15 nouveaux tests Vitest (Field 5 + Combobox 8 + catalog 7) — 60/60 verts. Documenté en `docs/design-system/07-form-completion.md`. 0 migration BDD, 0 dépendance npm.
