...
status: executing
stopped_at: "Phase 10.02 (SMS patients impactés lors d'une réaffectation — complément replanification) livrée localement. PR à ouvrir."
last_updated: "2026-06-12T20:00:00.000Z"
last_activity: Phase 10.02 (SMS patients impactés lors d'une réaffectation — complément replanification ; 1 migration template, 0 dépendance). CdG §5.14 l.368 : SMS aux patients dont la course change de chauffeur suite panne/indispo. Ferme le scénario panne (signalement → réaffectation → notification). D-01 migration 20260612000004 : seed additif idempotent template sms_templates key reaffectation (Mustache {{patient_prenom}}/{{date}}/{{heure}}, ≤160 car, éditable module sms-templates). D-02 branché dans reassignRidesBatchAction APRÈS commit, pour les courses dont driver_id a effectivement changé → helper best-effort notifyReassignedPatients (_lib/notify-reaffectation.ts, server-only) réutilise socle SMS 09.03 : getActiveSmsConsentMap (1 requête, anti-N+1) + isConsentValid (règle RGPD unique DEC-008) + renderTemplate + sendSms + trace sms_messages (queued/skipped_consent_revoked/failed), envois par lots de 10. Client service-role (sms_messages sans policy INSERT authenticated, SELECT only) → requêtes filtrées organization_id (defense in depth). D-03 best-effort : l'envoi ne rollback JAMAIS la réaffectation (try/catch global + per-envoi). D-04 pas d'ETA inventé sans géoloc HDS → horaire programmé maintenu. Idempotence : SMS seulement si driver_id changé (rejeu même chauffeur → 0 SMS). Garde-fous : consentement RGPD réutilisé (jamais réécrit), traçabilité intégrale, pas de N+1, 0 dépendance. typecheck+lint(0 err, 8 warn)+build verts. Ferme registre §8.1. DEC-161 LOCKED.
last_activity_prev: Phase 10.01 replanification dynamique cœur (DEC-160). Phase 07.04 récap PDF périodique B2B (DEC-159).
...
# + 10.02 SMS patients impactés lors d'une réaffectation (complément replanification) = 88. (06.40
...
# Dernière phase livrée : 10.02 (SMS patients impactés réaffectation). Dernier DEC : 161 (10.02). Dernier ADR : ADR-013 (06.33).
progress:
  total_phases: 90
  completed_phases: 88
  total_plans: 93
...
**Current focus:** Phase 10.02 (SMS patients impactés lors d'une réaffectation — complément replanification) livrée localement. Template `reaffectation` (seed additif idempotent, éditable module sms-templates) + helper best-effort `notifyReassignedPatients` branché dans `reassignRidesBatchAction` après commit, pour les courses dont le chauffeur a effectivement changé. Réutilise le socle SMS 09.03 (consentement RGPD, `sendSms`, trace `sms_messages`, envois par lots) ; client service-role filtré org ; ne rollback jamais la réaffectation ; ETA non promis sans géoloc HDS. Ferme le scénario panne (signalement → réaffectation → notification) et le registre §8.1. **GATE 08.x toujours en attente** : test isolation 2-orgs data-cache sur preview. 88 phases feature livrées + 3 lots hors compte (06.40 hygiène, 06.56 méthode, 06.67 chore CI) (cf. commentaire de comptage dans le frontmatter). 4 release toggles OFF pré-infra : GEOLOC, MESSAGING, UPLOAD_DOCS, EMAIL. Restantes : lots suivants replanification (registre §8 — réaffectation auto, suivi atelier, recalcul ETA, poussée realtime incidents, affectation au plus proche pour demande immédiate §8.6) ; lot alignement versions Supabase (débloque le typage des écritures, DEC-155) ; Lot 3 Suspense (différé) ; Phase 09 HDS (registre §1.1/§4.3 — débloque la précision géoloc du score replanification) + Phase 10 géoloc réelle ; choix provider email (registre §1.2) ; messagerie complète (registre §1.4) ; contacts donneurs multiples (registre §6.4) + portail self-service V1.5 (§6.5) ; étapes restantes du plan d'audit.
...
Phase: 10.02 (SMS patients impactés lors d'une réaffectation — complément replanification) livrée localement (2026-06-12). PR à ouvrir.
Phase next: lots suivants replanification (registre §8 — réaffectation auto, suivi atelier, recalcul ETA, poussée realtime, affectation au plus proche §8.6) ; lot alignement versions Supabase (typage écritures, DEC-155) ; valider l'isolation 2-orgs data-cache sur preview (08.x) ; Lot 3 Suspense (différé) ; choix provider email (registre §1.2) ; messagerie complète (registre §1.4) ; contacts donneurs multiples (§6.4) ; portail self-service V1.5 (§6.5) ; Phase 09 HDS ; Phase 10 géoloc.
Status: 88 phases feature + 3 lots hors compte. SMS réaffectation : template reaffectation (seed additif idempotent, éditable module sms-templates) + helper best-effort notifyReassignedPatients branché dans reassignRidesBatchAction après commit (courses dont driver_id a changé) ; réutilise socle SMS 09.03 (consentement RGPD getActiveSmsConsentMap+isConsentValid, sendSms, trace sms_messages queued/skipped/failed, envois par lots) ; client service-role filtré org (sms_messages sans policy INSERT authenticated) ; ne rollback jamais la réaffectation ; ETA non promis sans géoloc HDS ; idempotence (SMS si driver changé). Ferme scénario panne complet + registre §8.1. Data-cache référentiels 4/5 (gate preview en attente). 4 release toggles OFF pré-infra.
...
Last activity: Phase 10.02 (SMS patients impactés réaffectation). D-01 migration 20260612000004 : seed additif idempotent template sms_templates key reaffectation (Mustache {{patient_prenom}}/{{date}}/{{heure}}, ≤160 car, éditable). D-02 branché dans reassignRidesBatchAction APRÈS commit pour les courses dont driver_id a changé → helper best-effort notifyReassignedPatients (_lib/notify-reaffectation.ts, server-only) : socle SMS 09.03 (getActiveSmsConsentMap anti-N+1 + isConsentValid DEC-008 + renderTemplate + sendSms + trace sms_messages queued/skipped_consent_revoked/failed, lots de 10) ; client service-role (sms_messages SELECT-only) filtré org. D-03 best-effort : ne rollback jamais la réaffectation (try/catch global + per-envoi). D-04 pas d'ETA inventé sans géoloc HDS → horaire programmé. Idempotence : SMS si driver_id changé. Garde-fous : consentement RGPD réutilisé, traçabilité intégrale, pas de N+1, 0 dépendance. typecheck+lint(0 err, 8 warn)+build verts. 1 migration (template), 0 dépendance. Ferme registre §8.1. DEC-161 LOCKED. PR à ouvrir.
Précédent: 10.01 replanification dynamique cœur (DEC-160), 07.04 récap PDF périodique B2B (DEC-159), 07.03 demande groupée B2B (DEC-158).
