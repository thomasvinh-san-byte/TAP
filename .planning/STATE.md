...
status: executing
stopped_at: "Phase 11.01 (T5 — conformité détour transport partagé, décret 2025-202) livrée localement. PR à ouvrir."
last_updated: "2026-06-13T12:00:00.000Z"
last_activity: Phase 11.01 (T5 — contrainte de détour réglementaire sur la mutualisation, décret n°2025-202 du 28/02/2025 ; 0 migration, 0 dépendance). Trou de conformité (US-REG-02) : le décret encadre le transport partagé conventionné (détour ≤10 km/pers dès la 2ᵉ, ≤30 km total ; sinon non remboursable CGSS) — le solveur solve-local.ts optimisait le km à vide sans vérifier aucune limite. Faisable sans HDS (Haversine). D-01/D-02 fonction PURE checkRegulatoryDetour : détour/patient = distance en itinéraire partagé (pickup→dropoff en suivant la séquence) − trajet direct, en haversineKm×correctionFactor ; 1ᵉʳ patient pris en charge = RÉFÉRENCE (exempt limite individuelle + exclu du total, « à partir de la 2ᵉ ») ; constantes nommées DETOUR_MAX_PAR_PERSONNE_KM=10 / DETOUR_MAX_TOTAL_KM=30 (décret, pas de magic number). D-03 groupement non conforme NON proposé (option a, rejet entier : courses retirées de used → redeviennent individuelles, sans consommer de véhicule). D-04 compteur ecartesDetour + console.info (traçabilité). D-05 gain inchangé pour les groupes conformes (la contrainte FILTRE). NB modèle V1 : route séquentielle pickup→dropoff par patient (détour ≈0) → garde-fou qui mord dès que la route entrelace (post-HDS routing) ; précision exacte = routing réel (registre §4.1/§4.2). Tests solve-local.detour.test.ts (5) : séquentiel conforme, 2ᵉ patient >10 rejeté, 1ᵉʳ exempt, total >30 rejeté + régression solveur (groupe conforme proposé) — 18 tests solveur verts. typecheck+lint(0 err)+build verts. 0 migration, 0 dépendance. Résout T5. DEC-169 LOCKED.
last_activity_prev: Phase 09.04 fix concurrence anti-TOCTOU (DEC-168). Phase 06.69 push PWA chauffeur (DEC-167).
...
# Dernière phase livrée : 11.01 (conformité détour mutualisation, T5). Dernier DEC : 169 (11.01). Dernier ADR : ADR-013 (06.33).
progress:
  total_phases: 97
  completed_phases: 95
  total_plans: 101
...
Phase: 11.01 (T5 — conformité détour transport partagé, décret 2025-202) livrée localement (2026-06-12). PR à ouvrir.
Phase next: générer les clés VAPID (secrets Vercel) pour activer le push en prod (06.69) ; rebrancher le détour réglementaire sur le routing réel quand disponible (registre §4.2, post-HDS) ; KPIs 5.20 restants (registre §9.4 — écart prévu/réalisé, récurrentes/ponctuelles, KPIs économiques, occupation/productivité, litiges) ; lots suivants replanification (registre §8) ; transaction atomique demande groupée (registre §5, candidat RPC) ; lot alignement versions Supabase (typage écritures, DEC-155) ; valider l'isolation 2-orgs data-cache sur preview (08.x) ; Lot 3 Suspense (différé) ; choix provider email (registre §1.2) ; messagerie complète (registre §1.4) ; contacts donneurs multiples (§6.4) ; portail self-service V1.5 (§6.5) ; Phase 09 HDS ; Phase 10 géoloc.
Status: 95 phases feature + 4 lots hors compte. Conformité détour (décret 2025-202) : fonction pure checkRegulatoryDetour dans solve-local.ts (détour/patient = itinéraire partagé − direct, Haversine×correction ; 1ᵉʳ patient exempt ; ≤10 km/pers + ≤30 km total ; constantes nommées) ; groupement non conforme non proposé (rejet entier) ; gain inchangé ; traçabilité console.info. Approximation V1 (route séquentielle → détour ≈0) ; mord quand la route entrelace (post-HDS). 18 tests solveur verts (5 dédiés détour). Data-cache référentiels 5/6 (gate preview en attente). 4 release toggles OFF pré-infra.
...
Last activity: Phase 11.01 (T5 conformité détour mutualisation, décret 2025-202). Trou de conformité (US-REG-02) : détour transport partagé ≤10 km/pers dès la 2ᵉ + ≤30 km total, sinon non remboursable CGSS ; solveur ne vérifiait rien. D-01/D-02 fonction pure checkRegulatoryDetour (solve-local.ts) : détour/patient = itinéraire partagé − direct (haversineKm×correctionFactor) ; 1ᵉʳ patient = référence exempte ; constantes DETOUR_MAX_PAR_PERSONNE_KM=10 / DETOUR_MAX_TOTAL_KM=30. D-03 groupement non conforme non proposé (rejet entier, courses retirées de used). D-04 compteur + console.info. D-05 gain inchangé (filtre seul). NB route séquentielle V1 → détour ≈0 ; mord quand entrelacement (post-HDS) ; précision exacte = routing réel (registre §4.2). Tests solve-local.detour.test.ts (5) : séquentiel conforme, 2ᵉ>10 rejeté, 1ᵉʳ exempt, total>30 rejeté, régression groupe conforme ; 18 tests solveur verts. typecheck+lint(0 err)+build verts. 0 migration, 0 dépendance. Résout T5. DEC-169 LOCKED. PR à ouvrir.
Précédent: 09.04 fix concurrence anti-TOCTOU (DEC-168), 06.69 push PWA chauffeur (DEC-167), 07.09 KPI panier moyen (DEC-166).
