...
status: Phase 04.9 — Wave 1 livrée, ready Wave 2+3 parallèle
stopped_at: Phase 04.9 Wave 1 complete (PR #112) — ready Wave 2 (assets PWA) + Wave 3 (Serwist+Dexie) parallèle
last_updated: "2026-05-18T12:00:00.000Z"
last_activity: 2026-05-18 — Wave 1 Phase 04.9 livrée (PR #112). 2 Route Handlers POST /api/driver/rides/[rideId]/{start,end} créés avec auth cookies Supabase via getAuthContext() (compatible Route Handler) + idempotency UUID v4 dédupée 24h via table idempotency_keys (PRIMARY KEY composite user_id+mutation_type+resource_id+key + RLS self-only authenticated + index expires_at + comment table). Migration 20260518000002_idempotency_keys.sql appliquée via CD push DEC-032 (PAS MCP). Helpers centralisés apps/web/src/lib/api/ : requireDriverFromRouteHandler() (réutilise getAuthContext puis joint drivers.profile_id=auth.users.id + archive=false) et withIdempotency<T>() (check cache → MISS apply + INSERT response_json best-effort). Pattern startRideAction/endRideAction (conduite/actions.ts) reproduit avec adaptations : status guards assignee→en_cours et en_cours→terminee, ownership check driver_id=myDriverId, row count check DEC-041 (updated.length===0 → 403 RLS rejection détectée), refinement Zod payment_status='encaisse' ⇒ payment_method requis pour /end. Server Actions existantes intactes (fallback online direct browser). DÉCISION SCOPE : Vitest non configuré dans apps/web (seulement packages/pricing|shared) — setup serait 30-60 min hors scope Wave 1. Tests deferred Wave 6 E2E Playwright qui couvrira le flow offline complet exerçant ces endpoints. Validation Wave 1 : typecheck PASS (V8 stratégie CI V1.5), curl manuel local validé scénarios 6 codes attendus à effectuer post-merge sur preview Vercel. Prêt Wave 2 (assets PWA pwa-asset-generator) + Wave 3 (Serwist 9 + Dexie 4) en parallèle. Vélocité Wave 1 estimée 15-20 min ⇒ réel à mesurer. Précédent : PR #111 plan 7 waves.
progress:
  total_phases: 15
  completed_phases: 4
  total_plans: 24
