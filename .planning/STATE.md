...
last_activity: 2026-05-18 — Hotfix UX patient form V2 livré (PR #107) : BAN auto-remplissage code postal + ville après sélection adresse. Architecture AddressPickerField étendue avec callback onSelect optionnel (V1 backward compatible courses, export type BanSuggestion). PatientAddressField expose onAddressPick au parent. CoordinatesSection lift state postcode/ville partagé + key remount workaround sur CityPostalCodeField (composant uncontrolled actuel). Dette refactor forms composants contrôlés inscrite CONCERNS Phase 06. UI-PATTERNS enrichi section « Autocomplete adresse — Propagation détails V2 » + 4 anti-patterns. Patient form complet : Sexe FR médical + autocomplete BAN + auto-remplissage CP/ville. UX terrain réduit erreurs saisie. Précédent : Hotfix UX patient form V1 (PR #106 label Sexe + autocomplete BAN branché).
progress:
  total_phases: 15
  completed_phases: 4
  total_plans: 24
