...
**Optimizer status** : solveur heuristique **TypeScript natif** (`solveLocal`, `apps/web/src/lib/optimizer/`, testé), appelé **directement** par le Route Handler `/api/optimizer` — plus de microservice Python OR-Tools, plus d'appel HTTP, plus de mock. `OPTIMIZER_USE_MOCK` a été supprimé du code. Cluster-first / route-second, contrat zod `@tap/optimizer-client` inchangé (réversible). Réf ADR-010 (supersede ADR-008 + ADR-009), DEC-093.
...
Blockers: 1 en attente — (1) GATE isolation 2-orgs data-cache (08.x) sur preview. _(L'ancien blocker (2) « typage des payloads d'écriture = never → lot d'alignement de versions » est REQUALIFIÉ 2026-07-03 : ce n'est PAS un blocker. Constat stable de 13 casts d'écriture sûrs à l'exécution (payload erroné rattrapé par contraintes BDD + isolation org), à laisser en place. Cause exacte : `@supabase/postgrest-js` est embarqué par `@supabase/supabase-js` (client épinglé pour la LECTURE/auth), pas une dépendance de `@supabase/ssr` ; **monter les versions aggrave** (casse la lecture en plus de l'écriture, rapports amont). Déblocage éventuel = monter `@supabase/ssr` (touche cookies/auth, à risque) ou shim de type local, à revisiter quand l'amont aura mûri. Voir DEC-155 amendée + registre §5.)_
