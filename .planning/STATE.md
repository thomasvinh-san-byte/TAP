...
# Dernière phase livrée : 12.06 (fix cohérence ride_group météo). Dernier DEC : 175 (12.06). Dernier ADR : ADR-013 (06.33).
progress:
  total_phases: 99
  completed_phases: 97
  total_plans: 103
...
Phase: 12.06 (fix B3 — cohérence ride_group lors d'une annulation météo) livrée localement (2026-06-12). PR à ouvrir.
...
Status: 97 phases feature + 7 lots hors compte (06.40, 06.56, 06.67, 09.04, 12.04, 12.05, 12.06 fix). Audit transversal des effets de bord annulee_meteo CLOS : 12.04 B1 (affichage dashboard/export/badges) + 12.05 B2 (CRITIQUE compteur remboursement prescriptions) + 12.06 B3 (cohérence ride_group). Tous trous V1 traités sauf T4 2FA. Couplage app↔trigger SQL = seul point de vigilance manuel permanent. Data-cache référentiels 5/6 (gate preview en attente). 4 release toggles OFF pré-infra.
...
Last activity: Phase 12.06 (fix B3 cohérence ride_group annulation météo, effet de bord 12.01). L'annulation en lot météo ignorait ride_group_id → demande groupée B2B acceptee dont toutes les courses passent annulee_meteo restait acceptee (groupe fantôme). D-01 migration enum 20260613000005 (add value if not exists 'annulee' à ride_group_status). D-02 reconcileWeatherGroups anti-N+1 : groupes sans course survivante (validee/assignee/en_cours/terminee) → annulee via un update groupé .eq('status','acceptee') ; survie partielle reste acceptee. D-03 best-effort (jamais de rollback). D-04 aucune map UI de statut groupe à étendre (seule la file en_attente listée). typecheck+lint(0 err)+build verts. 1 migration enum, 0 dépendance. DEC-175 LOCKED. Clôt l'audit des effets de bord météo (B1/B2/B3). PR à ouvrir.
Précédent: 12.05 fix CRITIQUE trigger prescription annulee_meteo (DEC-174), 12.04 fix cohérence annulee_meteo (DEC-173), 12.03 accompagnant + mineur/référent légal (DEC-172).
