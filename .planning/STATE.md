...
status: executing
stopped_at: "Phase 07.05 (référentiel prescripteurs — préalable des prescriptions) livrée localement. PR à ouvrir."
last_updated: "2026-06-12T22:00:00.000Z"
last_activity: Phase 07.05 (référentiel prescripteurs — CdG §5.4, préalable des prescriptions ; 1 migration, 0 dépendance). Médecins/établissements à l'origine des prescriptions. 3 entités DISTINCTES : prescripteur (qui prescrit) ≠ donneur d'ordres (qui commande/paie §5.5) ≠ patient (transporté) — nouvelle table prescribers, aucune fusion. D-01 migration 20260612000005 : enum prescriber_type (medecin|etablissement) + table prescribers (nom NOT NULL, prenom null, type, rpps/adeli/finess null, specialite, contact_telephone/email, adresse, actif/archive, created_by) calquée ordering_parties ; RLS forcée par org : SELECT same_org / INSERT+UPDATE dirigeant OU régulateur / pas de DELETE (archivage logique) ; index (org,actif) partiel ; trigger updated_at + audit prescriber.* ; pgTAP 11. Validations format @tap/shared (modèle siretSchema) : rppsSchema 11 chiffres, adeliSchema/finessSchema 9 chiffres (format seul, optionnels). D-02 front (admin)/admin/prescripteurs/ répliquant donneurs-ordres : page (guard requireAdminOrRegulateurPage) + actions CRUD/archivage (requireAdminOrRegulateur, DEC-041) + _lib/cached-queries.ts (data-cache par org prescribers:${orgId}, service-role + filtre .eq('organization_id') explicite, revalidateTag) + _components (liste DataTable + form drawer, identifiants conditionnels au type : RPPS/ADELI médecin, FINESS établissement). Nav : entrée Prescripteurs (menu Gestion). D-03 volumétrie/top/patients suivis (l.181-184) = dépendent de §5.3, NON construits. Garde-fous : 3 entités distinctes ; RLS org ; data-cache filtré org ; pas de lien course/prescription ici (c'est §5.3). Dette : prescribers hors types.gen.ts → .from('prescribers' as never) + as never (DEC-155). typecheck+lint(0 err, 8 warn)+build verts ; pgTAP 11. Suite registre §9 : gestion des prescriptions §5.3. DEC-162 LOCKED.
last_activity_prev: Phase 10.02 SMS patients impactés réaffectation (DEC-161). Phase 10.01 replanification dynamique cœur (DEC-160).
...
# + 07.05 référentiel prescripteurs (CdG §5.4, préalable des prescriptions) = 89. (06.40
...
# Dernière phase livrée : 07.05 (référentiel prescripteurs). Dernier DEC : 162 (07.05). Dernier ADR : ADR-013 (06.33).
progress:
  total_phases: 91
  completed_phases: 89
  total_plans: 94
...
**Current focus:** Phase 07.05 (référentiel prescripteurs — CdG §5.4, préalable des prescriptions) livrée localement. Nouvelle table `prescribers` (médecin/établissement, RPPS/ADELI/FINESS, RLS org dirigeant+régulateur, pgTAP 11) + front `(admin)/admin/prescripteurs/` répliquant donneurs-ordres (CRUD + data-cache par org + liste/form). 3 entités DISTINCTES : prescripteur ≠ donneur d'ordres ≠ patient. Préalable de la gestion des prescriptions (§5.3, lot suivant). **GATE 08.x toujours en attente** : test isolation 2-orgs data-cache sur preview. 89 phases feature livrées + 3 lots hors compte (06.40 hygiène, 06.56 méthode, 06.67 chore CI) (cf. commentaire de comptage dans le frontmatter). 4 release toggles OFF pré-infra : GEOLOC, MESSAGING, UPLOAD_DOCS, EMAIL. Restantes : gestion des prescriptions §5.3 (registre §9 — compteur trajets, alertes 80%/expiré, sans scan HDS) ; lots suivants replanification (registre §8) ; lot alignement versions Supabase (débloque le typage des écritures, DEC-155) ; Lot 3 Suspense (différé) ; Phase 09 HDS (registre §1.1/§4.3) + Phase 10 géoloc réelle ; choix provider email (registre §1.2) ; messagerie complète (registre §1.4) ; contacts donneurs multiples (registre §6.4) + portail self-service V1.5 (§6.5) ; étapes restantes du plan d'audit.
...
Phase: 07.05 (référentiel prescripteurs — préalable des prescriptions) livrée localement (2026-06-12). PR à ouvrir.
Phase next: gestion des prescriptions §5.3 (registre §9 — compteur trajets, alertes 80%/expiré, sans scan HDS) ; lots suivants replanification (registre §8) ; lot alignement versions Supabase (typage écritures, DEC-155) ; valider l'isolation 2-orgs data-cache sur preview (08.x) ; Lot 3 Suspense (différé) ; choix provider email (registre §1.2) ; messagerie complète (registre §1.4) ; contacts donneurs multiples (§6.4) ; portail self-service V1.5 (§6.5) ; Phase 09 HDS ; Phase 10 géoloc.
Status: 89 phases feature + 3 lots hors compte. Référentiel prescripteurs : nouvelle table prescribers (enum prescriber_type medecin|etablissement, RPPS/ADELI/FINESS, RLS org forcée SELECT same_org/INSERT+UPDATE dirigeant+régulateur/pas de DELETE, pgTAP 11) ; validations format @tap/shared (RPPS 11 / ADELI 9 / FINESS 9 chiffres) ; front répliquant donneurs-ordres (page guard dirigeant+régulateur, CRUD/archivage, data-cache par org prescribers:${orgId}, liste/form avec identifiants conditionnels au type) ; nav Prescripteurs (menu Gestion). 3 entités distinctes. Volumétrie/top/patients suivis = dépendent de §5.3. Data-cache référentiels 5/6 (gate preview en attente). 4 release toggles OFF pré-infra.
...
Last activity: Phase 07.05 (référentiel prescripteurs §5.4). D-01 migration 20260612000005 : enum prescriber_type (medecin|etablissement) + table prescribers (nom NOT NULL, prenom null, type, rpps/adeli/finess null, specialite, contact, adresse, actif/archive) calquée ordering_parties ; RLS forcée org SELECT same_org/INSERT+UPDATE dirigeant OU régulateur/pas de DELETE ; trigger updated_at + audit prescriber.* ; pgTAP 11. Validations @tap/shared rppsSchema(11)/adeliSchema(9)/finessSchema(9) format seul, optionnels. D-02 front (admin)/admin/prescripteurs/ : page (requireAdminOrRegulateurPage) + actions CRUD/archivage (requireAdminOrRegulateur, DEC-041) + cached-queries (data-cache par org, service-role + filtre .eq org, revalidateTag) + liste/form (identifiants conditionnels au type). Nav Prescripteurs (menu Gestion). D-03 volumétrie/top/patients suivis dépendent de §5.3, NON construits. Garde-fous : 3 entités distinctes ; RLS org ; data-cache filtré org ; pas de lien course/prescription ici. Dette : prescribers hors types.gen.ts → as never (DEC-155). typecheck+lint(0 err, 8 warn)+build verts ; pgTAP 11. 1 migration, 0 dépendance. Suite registre §9 (prescriptions §5.3). DEC-162 LOCKED. PR à ouvrir.
Précédent: 10.02 SMS patients impactés réaffectation (DEC-161), 10.01 replanification dynamique cœur (DEC-160), 07.04 récap PDF périodique B2B (DEC-159).
