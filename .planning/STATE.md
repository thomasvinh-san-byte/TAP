...
# Dernière phase livrée : 09.05 (garde-fou CI synchro statuts annulés). Dernier DEC : 176 (09.05). Dernier ADR : ADR-013 (06.33).
progress:
  total_phases: 99
  completed_phases: 97
  total_plans: 103
...
Phase: 09.05 (chore — garde-fou CI synchro app↔trigger SQL des statuts annulés) livrée localement (2026-06-12). PR à ouvrir.
...
Status: 97 phases feature + 8 lots hors compte (06.40, 06.56, 06.67, 09.04, 12.04, 12.05, 12.06, 09.05). Audit transversal des effets de bord annulee_meteo CLOS (B1 12.04 / B2 12.05 / B3 12.06) + garde-fou CI 09.05 qui rend la régression B2 impossible (test d'alignement app↔trigger SQL). Le couplage app↔trigger SQL n'est PLUS un point de vigilance manuel (protégé par test CI). Tous trous V1 traités sauf T4 2FA. Data-cache référentiels 5/6 (gate preview en attente). 4 release toggles OFF pré-infra.
...
Last activity: Phase 09.05 (chore garde-fou CI synchro app↔trigger SQL des statuts annulés). Le bug B2 venait d'un couplage manuel (RIDE_CANCELLED_STATUSES @tap/shared ↔ array `cancelled` du trigger prescription, qu'un trigger SQL ne peut pas importer). D-01 test vitest cancelled-statuses-sync.test.ts : lit la migration la plus récente définissant l'array (parcours décroissant, repo-root via import.meta.url, aucun nom hardcodé), extrait les valeurs (regex tolérante), asserte set(SQL) === set(RIDE_CANCELLED_STATUSES) ∪ {brouillon} avec message clair. D-03 échec explicite si array introuvable. D-02 couplage documenté (constante + test). Vérifié vert sur main + « test du test » (faux statut → rouge). Tourne dans ci.yml (pnpm test → @tap/shared:test). typecheck+lint(0 err)+build verts. 0 migration, 0 dépendance runtime. DEC-176 LOCKED. PR à ouvrir.
Précédent: 12.06 fix B3 cohérence ride_group annulation météo (DEC-175), 12.05 fix CRITIQUE trigger prescription annulee_meteo (DEC-174), 12.04 fix cohérence annulee_meteo (DEC-173).
