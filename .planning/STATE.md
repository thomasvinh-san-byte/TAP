...
# Dernière phase livrée : 12.04 (fix cohérence annulee_meteo). Dernier DEC : 173 (12.04). Dernier ADR : ADR-013 (06.33).
progress:
  total_phases: 99
  completed_phases: 97
  total_plans: 103
...
Phase: 12.04 (fix — cohérence statut annulee_meteo, effet de bord 12.01) livrée localement (2026-06-12). PR à ouvrir.
...
Status: 97 phases feature + 5 lots hors compte (06.40, 06.56, 06.67, 09.04, 12.04 fix). Fix annulee_meteo : source unique RIDE_CANCELLED_STATUSES (@tap/shared) → dashboard compte les annulations météo, export/conduite/badges/PDF cohérents. Tous trous V1 traités sauf T4 2FA. Data-cache référentiels 5/6 (gate preview en attente). 4 release toggles OFF pré-infra.
...
Last activity: Phase 12.04 (fix cohérence statut annulee_meteo, effet de bord 12.01). La valeur d'enum annulee_meteo (12.01) manquait dans plusieurs énumérations en dur des statuts d'annulation → bug : le taux d'annulation du dashboard excluait les annulations météo (faux pendant un épisode cyclonique). D-01 source unique RIDE_CANCELLED_STATUSES (4 valeurs) + type RideCancelledStatus dans @tap/shared (validators/ride.ts). D-02 fonctionnel : queries-dashboard.ts (compte), export-rides.ts (filtre z.enum + label), conduite/_lib/queries.ts (union DriverRideStatus). D-03 cosmétique : ride-card/ride-detail (badge), ride-badges.tsx (label régulateur), PDF recap chauffeurs+donneurs. D-04 : brouillon géré ; setup-sql.ts snapshot frozen hors scope (canonical = migrations). Sémantique inchangée ; sites d'écriture (groups/cockpit/cancel) non touchés. Grep contrôle OK. typecheck+lint(0 err)+build verts. 0 migration, 0 dépendance. DEC-173 LOCKED. PR à ouvrir.
Précédent: 12.03 accompagnant + mineur/référent légal (DEC-172), 12.01 mode alerte météo / cyclone (DEC-170), 11.01 conformité détour mutualisation décret 2025-202 (DEC-169).
