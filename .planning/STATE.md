...
last_activity: 2026-05-15 — Hotfix Phase 04.7-bis-perf livré : indexes BDD (5 FK hot path + RLS wrapping (SELECT current_organization_id()) sur 21 policies hot path + search_patients RPC wrapping) + auth.uid() wrapping interne current_organization_id(). Baseline mesurée par dirigeant Firefox Network : recherche patient 828-1214ms → cible <100ms (gain ~95%), navigation /patients 721ms → <300ms (gain ~60%), navigation /courses 865ms+794-1021ms → <300ms combinés (gain ~70%). Migration via CD strict (DEC-032). Sémantique RLS préservée (uniquement wrapping fonctions stables). UI-PATTERNS.md enrichi 3 sections performance (BDD indexes obligatoires + SSR Server Components patterns + mesure outils minimaux V1.5). Items différés Phase 06 HDS : 23 policies tables non hot path (legal/*, audit_logs) + 20 FKs audit + Sentry + Web Vitals + Lighthouse + Index Advisor automatisé.
progress:
  total_phases: 15
  completed_phases: 4
  total_plans: 23
