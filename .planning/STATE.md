...
last_activity: 2026-05-18 — Hotfix Vercel + Supabase URLs custom domain livré : vercel.json simplifié (Project Settings dashboard source de vérité, suppression buildCommand/installCommand/outputDirectory/ignoreCommand) + Site URL Supabase aligné tap-web-brown.vercel.app + 4 wildcards Redirect URLs réajoutées (manuel dirigeant). Cause root double : Supabase Site URL désaligné → cookies session rejetés → boucle middleware infinie ; vercel.json override Project Settings → bannière jaune conflit dashboard. Fix infra uniquement (pas de code applicatif, V1/V2 stricts). UI-PATTERNS.md enrichi section « Déploiement custom domain Vercel » + checklist. Démo design partner enfin opérationnelle sur custom domain. Frictions perf Phase 04.7-bis-perf maintenant mesurables Firefox Network. Précédent : Hotfix Phase 04.7-bis-perf (RLS wrapping (SELECT current_organization_id()) sur 21 policies + 5 FK indexes hot path + search_patients RPC wrapping, baseline 828-1214ms → cible <100ms recherche, 721-1021ms → <300ms navigation).
progress:
  total_phases: 15
  completed_phases: 4
  total_plans: 23
