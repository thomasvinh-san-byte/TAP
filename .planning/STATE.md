...
status: executing
stopped_at: "Phase 09.04 (fix concurrence anti-TOCTOU sur les transitions de statut de course) livrée localement. PR à ouvrir."
last_updated: "2026-06-13T10:00:00.000Z"
last_activity: Phase 09.04 (fix concurrence — anti-TOCTOU sur les transitions de statut de course ; 0 migration, 0 dépendance, hors compte feature). Trou de conception (sources Doyensec/EnterpriseDB) : l'affectation lisait status (select) puis écrivait assignee (read-then-write non atomique) → 2 régulateurs concurrents passent la vérif, le 2nd écrase silencieusement (TOCTOU/lost update) ; le row count check DEC-041 (.eq('id')) ne l'attrape pas. Solution = optimistic locking / compare-and-set (conflits rares → pas de verrou pessimiste) : statut attendu dans le WHERE de l'UPDATE → statut changé = 0 ligne = refus. D-01/D-02 WHERE durcis : assignRideAction .eq('status','validee') ; unassignRideAction .eq('status','assignee') ; startRideAction (chauffeur) .eq('status','assignee') ; endRideAction .eq('status','en_cours'). Déjà protégés (inchangés) : cancelRideAction + reassignRidesBatchAction (.in(status) dans le WHERE) ; idempotence réaffectation préservée. Non sur-protégés (D-02) : assignVehicle/override/payment/edit = mises à jour de CHAMPS, pas de transition du state machine status. D-03 message conflit explicite (« affectée par un autre régulateur, actualisez », distinct de droits insuffisants). Garde-fous : pas de FOR UPDATE/transaction lourde ; règles métier des transitions inchangées ; row count check DEC-041 réutilisé. typecheck+lint(0 err)+build verts. 0 migration, 0 dépendance. DEC-168 LOCKED.
last_activity_prev: Phase 06.69 push PWA chauffeur (DEC-167). Phase 07.09 KPI panier moyen (DEC-166).
...
# Dernière phase livrée : 09.04 (fix concurrence anti-TOCTOU, hors compte feature). Dernier DEC : 168 (09.04). Dernier ADR : ADR-013 (06.33).
progress:
  total_phases: 96
  completed_phases: 94
  total_plans: 100
...
Phase: 09.04 (fix concurrence anti-TOCTOU sur les transitions de statut de course) livrée localement (2026-06-12). PR à ouvrir.
Phase next: générer les clés VAPID (secrets Vercel) pour activer le push en prod (06.69) ; KPIs 5.20 restants (registre §9.4 — écart prévu/réalisé [préalable CA prévisionnel], récurrentes/ponctuelles [préalable activer rides.ride_recurrence_id], KPIs économiques, occupation/productivité, litiges) ; lots suivants replanification (registre §8) ; transaction atomique demande groupée (registre §5, candidat RPC) ; lot alignement versions Supabase (typage écritures, DEC-155) ; valider l'isolation 2-orgs data-cache sur preview (08.x) ; Lot 3 Suspense (différé) ; choix provider email (registre §1.2) ; messagerie complète (registre §1.4) ; contacts donneurs multiples (§6.4) ; portail self-service V1.5 (§6.5) ; Phase 09 HDS ; Phase 10 géoloc.
Status: 94 phases feature + 4 lots hors compte. Anti-TOCTOU : UPDATE conditionnel `.eq('status', <attendu>)` + row count check (optimistic locking) sur assignRideAction (validee)/unassignRideAction (assignee)/startRideAction (assignee)/endRideAction (en_cours) ; cancel/reassign déjà protégés (.in(status)) ; assignVehicle/override/payment/edit non sur-protégés (champs, pas transitions) ; message conflit explicite ; idempotence réaffectation préservée. Pas de verrou pessimiste. Data-cache référentiels 5/6 (gate preview en attente). 4 release toggles OFF pré-infra.
...
Last activity: Phase 09.04 (fix concurrence anti-TOCTOU statuts de course). Trou TOCTOU : read-then-write non atomique sur l'affectation → écrasement silencieux d'une affectation concurrente ; row count check DEC-041 (.eq('id')) insuffisant. D-01/D-02 compare-and-set : statut attendu ajouté au WHERE de l'UPDATE → assignRideAction .eq('status','validee'), unassignRideAction/startRideAction .eq('status','assignee'), endRideAction .eq('status','en_cours') ; cancel/reassign déjà protégés ; assignVehicle/override/payment/edit non sur-protégés (champs). D-03 message conflit explicite « affectée par un autre régulateur, actualisez ». D-04 idempotence réaffectation préservée. Garde-fous : pas de FOR UPDATE/transaction lourde ; règles métier inchangées ; row count check réutilisé. Registre §5 : tracé « transaction atomique demande groupée » (candidat RPC). typecheck+lint(0 err)+build verts. 0 migration, 0 dépendance. DEC-168 LOCKED. PR à ouvrir.
Précédent: 06.69 push PWA chauffeur (DEC-167), 07.09 KPI panier moyen (DEC-166), 07.08 tops commerciaux dirigeant (DEC-165).
