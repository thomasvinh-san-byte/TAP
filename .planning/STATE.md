...
status: executing
stopped_at: "Phase 10.01 (replanification dynamique — cœur panne/indispo + réaffectation scorée validable) livrée localement. PR à ouvrir."
last_updated: "2026-06-12T18:00:00.000Z"
last_activity: Phase 10.01 (replanification dynamique cœur — V1.5 §11.3 / §5.14 ; 1 migration, 0 dépendance). Chauffeur en panne/indisponible → la régulation réaffecte ses courses restantes. Score Haversine V1 SANS géoloc HDS. D-01 migration 20260612000003 : table driver_incidents (enum driver_incident_type panne_vehicule|indisponible, FK org+driver, nature/lieu, started_at, resolved_at NULL=ouvert, RLS org forcée : select same_org / insert régulateur+dirigeant OU chauffeur pour SON driver / update régulateur+dirigeant (résolution) / pas de DELETE, index partiel ouverts, pgTAP 12). Statut opérationnel temporaire ≠ drivers.actif (administratif permanent, jamais touché). D-02 signalement : panne côté PWA chauffeur (reportBreakdownAction, bouton ≥56px /conduite → nature+lieu) + indispo côté régulateur (declareIncidentAction /replanification). D-03 module PUR lib/replanning/reassign.ts (Vitest 5) : candidats = chauffeurs actifs sans incident ouvert, compatibles (réutilise isCompatible @tap/shared DEC-038 via map mode→type véhicule) ; score = Haversine (point réf candidat = sa course la plus proche dans le temps, sinon dépôt org, sinon inconnu) + pénalité de charge (+5 km/course). D-04 page /replanification : par incident, courses + proposition (Select pré-réglé sur meilleur, modifiable), réaffectation lot (reassignRidesBatchAction : driver→nouveau, vehicle→null, statut assignee, row-count DEC-041, courses en_cours/terminee jamais touchées) + Marquer résolu. D-05 course sans candidat → badge Non réaffectable (jamais d'auto-annulation). Alerte régulateur : incidents ouverts remontés cockpit (event_type driver_incident, au chargement). Nav régulateur : Replanification (7 entrées). Dette : driver_incidents hors types.gen.ts → .from('driver_incidents' as never) + as never payloads (DEC-155). typecheck+lint(0 err, 8 warn)+build verts ; Vitest 5 ; pgTAP 12. DEC-160 LOCKED.
last_activity_prev: Phase 07.04 récap PDF périodique B2B (DEC-159). Phase 07.03 demande groupée B2B (DEC-158).
...
# + 10.01 replanification dynamique cœur (panne/indispo + réaffectation scorée) = 87. (06.40
...
# Dernière phase livrée : 10.01 (replanification dynamique cœur). Dernier DEC : 160 (10.01). Dernier ADR : ADR-013 (06.33).
progress:
  total_phases: 89
  completed_phases: 87
  total_plans: 92
...
**Current focus:** Phase 10.01 (replanification dynamique — cœur panne/indispo + réaffectation scorée validable) livrée localement. Table `driver_incidents` (panne PWA chauffeur / indispo régulation, RLS org, pgTAP 12) ; module pur `lib/replanning/reassign.ts` (score Haversine + charge SANS géoloc HDS, réutilise `isCompatible`, Vitest 5) ; page régulateur `/replanification` (proposition par course validable manuellement/en lot, courses en_cours/terminee jamais touchées, badge « Non réaffectable » si aucun candidat) ; alerte cockpit `driver_incident`. Statut opérationnel ≠ `drivers.actif`. **GATE 08.x toujours en attente** : test isolation 2-orgs data-cache sur preview. 87 phases feature livrées + 3 lots hors compte (06.40 hygiène, 06.56 méthode, 06.67 chore CI) (cf. commentaire de comptage dans le frontmatter). 4 release toggles OFF pré-infra : GEOLOC, MESSAGING, UPLOAD_DOCS, EMAIL. Restantes : lots suivants replanification (registre §8 — SMS patients impactés, réaffectation auto, suivi atelier, recalcul ETA, poussée realtime) ; lot alignement versions Supabase (débloque le typage des écritures, DEC-155) ; Lot 3 Suspense (différé) ; Phase 09 HDS (registre §1.1/§4.3 — débloque la précision géoloc du score replanification) + Phase 10 géoloc réelle ; choix provider email (registre §1.2) ; messagerie complète (registre §1.4) ; contacts donneurs multiples (registre §6.4) + portail self-service V1.5 (§6.5) ; étapes restantes du plan d'audit.
...
Phase: 10.01 (replanification dynamique — cœur panne/indispo + réaffectation scorée validable) livrée localement (2026-06-12). PR à ouvrir.
Phase next: lots suivants replanification (registre §8) ; lot alignement versions Supabase (typage écritures, DEC-155) ; valider l'isolation 2-orgs data-cache sur preview (08.x) ; Lot 3 Suspense (différé) ; choix provider email (registre §1.2) ; messagerie complète (registre §1.4) ; contacts donneurs multiples (§6.4) ; portail self-service V1.5 (§6.5) ; Phase 09 HDS ; Phase 10 géoloc.
Status: 87 phases feature + 3 lots hors compte. Replanification cœur : table driver_incidents (panne PWA chauffeur / indispo régulation, enum panne_vehicule|indisponible, RLS org forcée, pgTAP 12) ; module pur reassign.ts (score Haversine réf temporelle + dépôt + pénalité charge, sans géoloc, réutilise isCompatible, Vitest 5) ; page /replanification (proposition par course validable manuellement/lot, courses en_cours/terminee intouchables, badge Non réaffectable) ; signalement panne PWA + indispo régulateur ; alerte cockpit driver_incident. Statut opérationnel ≠ drivers.actif. Data-cache référentiels 4/5 (gate preview en attente). 4 release toggles OFF pré-infra.
...
Last activity: Phase 10.01 (replanification dynamique cœur). D-01 migration 20260612000003 : table driver_incidents (enum driver_incident_type, FK org+driver, nature/lieu, resolved_at NULL=ouvert, RLS org forcée select same_org/insert régulateur+dirigeant OU chauffeur pour son driver/update régulateur+dirigeant/pas de DELETE, index partiel ouverts, pgTAP 12) ; statut temporaire ≠ drivers.actif. D-02 signalement panne PWA chauffeur (reportBreakdownAction, bouton ≥56px) + indispo régulateur (declareIncidentAction). D-03 module PUR lib/replanning/reassign.ts (Vitest 5) : candidats actifs sans incident ouvert, compatibles (isCompatible @tap/shared), score Haversine (réf temporelle/dépôt) + pénalité charge. D-04 page /replanification : proposition par course (Select modifiable) + réaffectation lot (reassignRidesBatchAction : driver→nouveau, vehicle→null, statut assignee, DEC-041 ; en_cours/terminee intouchables) + Marquer résolu. D-05 sans candidat → Non réaffectable. Alerte cockpit driver_incident (chargement). Nav régulateur Replanification. Dette : driver_incidents hors types.gen.ts → as never (DEC-155). typecheck+lint(0 err, 8 warn)+build verts ; Vitest 5 ; pgTAP 12. 1 migration, 0 dépendance. DEC-160 LOCKED. PR à ouvrir.
Précédent: 07.04 récap PDF périodique B2B (DEC-159), 07.03 demande groupée B2B (DEC-158), 07.02 grille tarifaire B2B (DEC-157).
