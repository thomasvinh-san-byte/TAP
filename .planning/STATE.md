...
last_activity: 2026-05-15 — Hotfix Phase 04.7-bis élargi livré : courses table truncation pattern Linear/Stripe + chauffeurs raccroché layout (app) + patients archivage soft-delete pattern healthcare HSE/HIPAA. UI-PATTERNS.md enrichi 3 sections (tables denses overflow + soft-delete healthcare + layout unique config-driven). Démo régulatrice utilisable terrain quotidien. Coût ~50 min réel vs ~2h estimé. Phase 04.7 clôturée complètement (execute + UAT informel + hotfix 04.7-bis initial + hotfix élargi + verify-work). 11 PR Phase 04.7 cumulées (#88 Discuss, #90 UI-SPEC, #91 Plans + DEC-042..044 LOCKED, #92 Wave A migration géocoding, #93 PLAN-1 pricing+Surface A/B+Override, #94 PLAN-2 caisse complète, #95 PLAN-3 threading+backfill, #96 closure execute, #97 méthodologie UAT informel obligatoire VISION.md, #98 hotfix-bis modal+filtre date+pagination courses, #99 verify-work UAT+SUMMARY). Vélocité Phase 04.7 -85% (~45 min exécution / 4-5.5h estimé) + ~25 min hotfix-bis + ~30 min verify = ~1h40 cumulé total. Méthodologie « pipeline GSD étendu — UAT informel obligatoire » validée par premier cas concret (3 frictions invisibles à la spec révélées : modal width + filtre date + pagination courses). Cumul V1.5 (Phase 04.5 + 04.7) : 22 PR livrées, ~6h réel vs ~19-25h estimé, vélocité globale -76%. Démo design partner business prête. Items différés Phase 04.9 (intégration end-ride-modal chauffeur, vue chauffeur encaissements, tests E2E PLAN-3) + Phase 06 (audit RLS/SA systémique, NIR Edge Function 401, 3 dettes CI V1.5). Prêt /gsd-discuss-phase 04.9 PWA chauffeur enveloppe (8-10h estimé / 2.5-3h réel projeté).
...
last_activity: 2026-05-18 — Hotfix UX racine livré : apps/web/src/app/page.tsx Server Component redirect /login ou /patients selon auth. Élimine 404 brut Next.js par défaut sur tap-web-brown.vercel.app (friction démo design partner). Documentation leçons méta marathon Vercel custom domain inscrites CONCERNS.md (sur-diagnostic à éviter, Pattern A Vercel, Supabase custom domain checklist, règle « 3 hypothèses sans convergence → STOP audit fresh ») + UI-PATTERNS.md section « Custom domain Vercel + Supabase Auth » + anti-patterns. Items annulés inscrits : Migration 20260518000001 SECURITY DEFINER ANNULÉE conceptuellement (théorie sur-diagnostiquée, migration restée mergée car restaure état foundations.sql original sans effet de bord). Analyse perf post-PR #101 : aucun gain mesurable sur tap-web-brown (820ms médiane navigation /patients), cible <300ms NON ATTEINTE — Phase 04.7-perf-v2 séparée à ouvrir. Cumul session marathon Vercel custom domain : PR #100-#104. Phase 04.7-bis complète et démontrable design partner. Précédent : Hotfix CRITIQUE régression RLS récursion (théorie annulée a posteriori).
progress:
  total_phases: 15
  completed_phases: 4
  total_plans: 23
