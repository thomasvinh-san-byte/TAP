...
status: executing
stopped_at: "Phase 07.06 (prescriptions / bons de transport — logique compteur + alertes, sans scan) livrée localement. PR à ouvrir."
last_updated: "2026-06-13T00:00:00.000Z"
last_activity: Phase 07.06 (prescriptions / bons de transport — CdG §5.3, fort enjeu remboursement ; 1 migration, 0 dépendance). Toute la logique métier SANS le scan (reporté Phase 09 HDS). D-01 migration 20260612000006 : enum prescription_status (active|epuisee|expiree) + table prescriptions (patient_id NOT NULL, prescriber_id null FK 07.05, numero, date_prescription, finess/motif/type_transport, trajets_autorises check>0, trajets_consommes default 0, date_expiration, statut, document_url PRÉVU scan laissé NULL) ; RLS forcée org SELECT same_org/INSERT+UPDATE dirigeant+régulateur/pas de DELETE ; trigger updated_at + audit ; pgTAP 14 (RLS + compteur). D-02 rides.prescription_id NULLABLE (course non conventionnée OK) threadé rideExpressInputSchema + create/update (spread ...input) + prefill (RIDE_COLUMNS + RideRow). D-03 compteur idempotent via TRIGGER Postgres rides_prescription_counter (SECURITY DEFINER, delta sur transition état consommateur = ni brouillon ni annulée ; +1 devient consommateur, -1 annulation/réaffectation ; recalcul statut) — source unique, rejeu = 0 delta (prouvé pgTAP). D-04 alertes pures derivePrescriptionAlerts (@tap/shared, Vitest 6) : 80%/épuisé/expiré/renouvellement (récurrent + <15j) → cockpit PrescriptionAlertsPanel (getPrescriptionAlerts, dégradation gracieuse) + badges fiche patient. D-05 section Prescriptions fiche patient (liste + form, actions listPrescriptionsForPatientAction/createPrescriptionAction guard dirigeant+régulateur) + picker express PrescriptionPickerField (prescriptions du patient, avertissement non bloquant si épuisé/expiré). D-06 document_url NULL, aucun Storage. Garde-fous : pas de scan ; prescription_id nullable (obligation par avertissement, pas contrainte) ; compteur idempotent (donnée remboursement) ; 3 entités distinctes ; RLS org ; patients_safe. Dette : prescriptions/prescription_id hors types.gen.ts → as never (DEC-155). typecheck+lint(0 err, 8 warn)+build verts ; Vitest 6 ; pgTAP 14. Suite registre §9.2/§9.3 : volumétrie/top + KPIs conformité bons ; scan = Phase 09. DEC-163 LOCKED.
last_activity_prev: Phase 07.05 référentiel prescripteurs (DEC-162). Phase 10.02 SMS patients impactés réaffectation (DEC-161).
...
# + 07.06 prescriptions/bons de transport (compteur + alertes, sans scan) = 90. (06.40
...
# Dernière phase livrée : 07.06 (prescriptions/bons de transport). Dernier DEC : 163 (07.06). Dernier ADR : ADR-013 (06.33).
progress:
  total_phases: 92
  completed_phases: 90
  total_plans: 95
...
**Current focus:** Phase 07.06 (prescriptions / bons de transport — logique compteur + alertes, SANS scan) livrée localement. Table `prescriptions` (patient + prescripteur 07.05, trajets autorisés/consommés, statut active/epuisee/expiree, RLS org, pgTAP 14) + `rides.prescription_id` nullable ; **compteur de trajets idempotent via trigger Postgres** + alertes pures (80%/épuisé/expiré/renouvellement, Vitest 6) surfacées cockpit + fiche patient ; saisie structurée fiche patient + picker express avec avertissement non bloquant. Scan reporté Phase 09 HDS (document_url NULL). **GATE 08.x toujours en attente** : test isolation 2-orgs data-cache sur preview. 90 phases feature livrées + 3 lots hors compte (06.40 hygiène, 06.56 méthode, 06.67 chore CI) (cf. commentaire de comptage dans le frontmatter). 4 release toggles OFF pré-infra : GEOLOC, MESSAGING, UPLOAD_DOCS, EMAIL. Restantes : volumétrie/top prescripteurs (registre §9.2) + KPIs conformité bons (§9.3) ; lots suivants replanification (registre §8) ; lot alignement versions Supabase (débloque le typage des écritures, DEC-155) ; Lot 3 Suspense (différé) ; Phase 09 HDS (registre §1.1/§4.3 — débloque le scan des bons + précision géoloc replanification) + Phase 10 géoloc réelle ; choix provider email (registre §1.2) ; messagerie complète (registre §1.4) ; contacts donneurs multiples (registre §6.4) + portail self-service V1.5 (§6.5) ; étapes restantes du plan d'audit.
...
Phase: 07.06 (prescriptions / bons de transport — logique compteur + alertes, sans scan) livrée localement (2026-06-12). PR à ouvrir.
Phase next: volumétrie/top prescripteurs (registre §9.2) + KPIs conformité bons (§9.3) ; lots suivants replanification (registre §8) ; lot alignement versions Supabase (typage écritures, DEC-155) ; valider l'isolation 2-orgs data-cache sur preview (08.x) ; Lot 3 Suspense (différé) ; choix provider email (registre §1.2) ; messagerie complète (registre §1.4) ; contacts donneurs multiples (§6.4) ; portail self-service V1.5 (§6.5) ; Phase 09 HDS ; Phase 10 géoloc.
Status: 90 phases feature + 3 lots hors compte. Prescriptions/bons : table prescriptions (patient + prescripteur 07.05, trajets_autorises/consommes, statut active/epuisee/expiree, RLS org dirigeant+régulateur, pgTAP 14) + rides.prescription_id nullable ; compteur de trajets IDEMPOTENT via trigger Postgres (delta sur transition état consommateur) ; alertes pures derivePrescriptionAlerts (Vitest 6) → cockpit PrescriptionAlertsPanel + badges fiche patient ; saisie structurée fiche patient + picker express avertissement non bloquant. Scan = Phase 09 HDS (document_url NULL). 3 entités distinctes. Data-cache référentiels 5/6 (gate preview en attente). 4 release toggles OFF pré-infra.
...
Last activity: Phase 07.06 (prescriptions/bons §5.3). D-01 migration 20260612000006 : enum prescription_status + table prescriptions (patient NOT NULL, prescriber null FK, numero, date, trajets_autorises/consommes, date_expiration, statut, document_url NULL pré-scan) ; RLS forcée org dirigeant+régulateur/pas de DELETE ; pgTAP 14. D-02 rides.prescription_id nullable threadé schema+create+edit+prefill (RIDE_COLUMNS+RideRow). D-03 compteur idempotent via trigger Postgres rides_prescription_counter (SECURITY DEFINER, delta sur transition état consommateur, recalcul statut). D-04 alertes pures derivePrescriptionAlerts (@tap/shared, Vitest 6) → cockpit PrescriptionAlertsPanel (getPrescriptionAlerts, dégradation gracieuse) + badges fiche patient. D-05 section fiche patient (listPrescriptionsForPatientAction/createPrescriptionAction, guard dirigeant+régulateur) + picker express PrescriptionPickerField (avertissement non bloquant si épuisé/expiré). D-06 document_url NULL, aucun Storage. Garde-fous : pas de scan ; prescription_id nullable ; compteur idempotent ; 3 entités distinctes ; RLS org ; patients_safe. Dette : prescriptions/prescription_id hors types.gen.ts → as never (DEC-155). typecheck+lint(0 err, 8 warn)+build verts ; Vitest 6 ; pgTAP 14. 1 migration, 0 dépendance. Suite registre §9.2/§9.3. DEC-163 LOCKED. PR à ouvrir.
Précédent: 07.05 référentiel prescripteurs (DEC-162), 10.02 SMS patients impactés réaffectation (DEC-161), 10.01 replanification dynamique cœur (DEC-160).
