...
# Dernière phase livrée : 13.01 (mode dégradé régulateur). Dernier DEC : 177 (13.01). Dernier ADR : ADR-013 (06.33).
progress:
  total_phases: 100
  completed_phases: 98
  total_plans: 104
...
Phase: 13.01 (mode dégradé régulateur — consultation hors-ligne planning J/J+1) livrée localement (2026-06-12). PR à ouvrir.
...
Status: 98 phases feature + 8 lots hors compte (06.40, 06.56, 06.67, 09.04, 12.04, 12.05, 12.06, 09.05). Mode dégradé 5.24 COMPLET (chauffeur + régulateur 13.01). Audit des effets de bord annulee_meteo clos (B1/B2/B3) + garde-fou CI 09.05. Tous trous V1 traités sauf T4 2FA. Restants = chantiers à préalable (KPIs économiques/opérationnels) + blocages business (HDS, email, Lomaco). Data-cache référentiels 5/6 (gate preview en attente). 4 release toggles OFF pré-infra.
...
Last activity: Phase 13.01 (mode dégradé régulateur — consultation hors-ligne planning J/J+1 lecture seule). Dernier trou de 5.24. Régulateur LECTURE SEULE (≠ chauffeur qui écrit offline) → réutilise les mécanismes (Dexie, listeners, SW), pas la sémantique d'écriture. D-01 store Dexie séparé tap-regulateur-offline (planning_mirror + contacts patient/chauffeur). D-02 OfflineGate alimente le miroir (J+J+1) quand en ligne via getRegulateurPlanningMirrorAction (guard dirigeant/régulateur, RLS). D-03 péremption 4h stricte (isPlanningStale) : planning masqué + message si >4h. D-04 DegradedPlanning lecture seule (bandeau, contacts tel:). D-05 SW NetworkFirst /cockpit (sans régresser /conduite ni chauffeur). D-06 useOnline. RGPD : cache vidé à la déconnexion (UserMenu), masqué >4h, scope org. typecheck+lint(0 err)+build(SW OK) verts. 0 migration BDD, 0 dépendance. Complète 5.24. DEC-177 LOCKED. PR à ouvrir.
Précédent: 09.05 garde-fou CI synchro app↔trigger SQL (DEC-176), 12.06 fix B3 cohérence ride_group annulation météo (DEC-175), 12.05 fix CRITIQUE trigger prescription annulee_meteo (DEC-174).
