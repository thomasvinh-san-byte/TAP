...
# Dernière phase livrée : 12.03 (accompagnant + mineur/référent, T2+T3). Dernier DEC : 172 (12.03). Dernier ADR : ADR-013 (06.33).
progress:
  total_phases: 99
  completed_phases: 97
  total_plans: 103
...
Phase: 12.03 (T2+T3 — accompagnant + mineur/référent légal, sans scan) livrée localement (2026-06-12). PR à ouvrir.
...
Status: 97 phases feature + 4 lots hors compte. Accompagnant : colonnes rides + supplément paramétrable via grille CGSS (moteur 100 % branches maintenu, B2B dégradation gracieuse), bloc de saisie + affichage tarif. Mineur/tutelle : référent légal patient + statut dérivé isMinor + avertissement non bloquant à la saisie (référent lu base patients, patients_safe inchangée). Scan reporté HDS. Tous trous V1 traités sauf T4 2FA. Data-cache référentiels 5/6 (gate preview en attente). 4 release toggles OFF pré-infra.
...
Last activity: Phase 12.03 (T2+T3 accompagnant + mineur/référent légal, sans scan). US-REG-10. T2 accompagnant (CdG l.293) : colonnes rides.accompagnant/accompagnant_payant/accompagnant_identite + bloc saisie ; coût via la grille (tariff_grids.supplement_accompagnant_eur, moteur computeCgssFromDistance inchangé sauf terme additif soumis à majoration ; champ optionnel TariffGrid → grilles B2B dégradation gracieuse ?? 0) ; affiché PricingBreakdown + carte grille + formulaire grille CGSS ; recalc maintenance CGSS+B2B threade accompagnant_payant. Règle CPAM « même taux » → supplément paramétrable, à valider métier (tracé). Capacité véhicule tracée (contrat optimizer non modifié). T3 mineur/tutelle (CdG l.139-140) : colonnes patients.referent_nom/lien/telephone/type (check) + referent_document_url NULL (scan reporté HDS) ; statut mineur dérivé de date_naissance (util isMinor @tap/shared, jamais stocké) ; avertissement non bloquant à la saisie course si référent manquant (MinorReferentWarning, pattern bon expiré 07.06 ; statut serveur getPatientReferentStatusAction base patients non-NIR). patients_safe inchangée (dépendance search_patients select p.*) → référent lu base ; edit prefill via getPatientReferentFields. Tests Vitest isMinor (8) + pricing accompagnant (3, 100 % branches = 18). typecheck+lint(0 err)+build verts. 3 migrations additives, 0 dépendance. Résout T2+T3 (sans scan). DEC-172 LOCKED. PR à ouvrir.
Précédent: 12.01 mode alerte météo / cyclone (DEC-170), 11.01 conformité détour mutualisation décret 2025-202 (DEC-169), 09.04 fix concurrence anti-TOCTOU (DEC-168).
