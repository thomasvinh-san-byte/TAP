...
status: executing
stopped_at: "Phase 07.04 (récap PDF périodique B2B — extension donneurs d'ordres 3/3) livrée localement. PR à ouvrir. Chaîne B2B complète."
last_updated: "2026-06-12T16:00:00.000Z"
last_activity: Phase 07.04 (récap PDF périodique B2B — extension donneurs d'ordres 3/3, dernière ; 0 migration, 0 dépendance). CdC §5.5 l.195 : récapitulatif périodique PDF pour la facturation centralisée. Boucle la chaîne B2B (grille 07.02 → groupée 07.03 → récap PDF). D-01 route api/admin/donneurs-ordres/recap/pdf calquée sur recap chauffeur (DEC-116) : params ordering_party/from/to, runtime nodejs, guard dirigeant+régulateur (403 sinon), isolation org (donneur lu via RLS → cross-org 404), courses 1 requête .eq('ordering_party_id').eq('archive',false).gte/lte + patients .in() sur patients_safe (pas de N+1, RGPD), renderToStream, headers téléchargement, audit log ordering_party.recap.exported_pdf avant rendu. D-02 composant RecapDonneurPdf réutilise PdfDocument/pdfStyles (charte commune) : en-tête raison sociale+SIRET, résumé (nb/dont terminées/total €), tableau date/patient/trajet/statut/tarif ; total € = tarif_amount_eur stocké (reflète grille B2B 07.02 si grille_propre, PAS de recalcul). D-03 bouton « Récap PDF » dans fiche donneur (édition), Dialog période pré-remplie selon modalite_facturation (hebdo → lundi semaine en cours ; mensuelle/a_la_course → 1er du mois) → today, ouvre route nouvel onglet. D-04 livré à la demande ; récap automatique planifié (cron + envoi email) tracé lot futur dépendant EMAIL_ENABLED (registre §6.3). Garde-fous : charte PDF commune ; pas de N+1 ; dirigeant/régulateur + isolation org ; tarif non recalculé ; patients_safe. typecheck+lint(0 err, 8 warn)+build verts. Chaîne d'extensions B2B COMPLÈTE (cœur 07.01 + grille 07.02 + groupée 07.03 + récap 07.04). DEC-159 LOCKED.
last_activity_prev: Phase 07.03 demande groupée B2B (DEC-158). Phase 07.02 grille tarifaire B2B (DEC-157).
...
# + 07.04 récap PDF périodique B2B (extension donneurs d'ordres 3/3) = 86. (06.40
...
# Dernière phase livrée : 07.04 (récap PDF périodique B2B). Dernier DEC : 159 (07.04). Dernier ADR : ADR-013 (06.33).
progress:
  total_phases: 88
  completed_phases: 86
  total_plans: 91
...
**Current focus:** Phase 07.04 (récap PDF périodique B2B — extension donneurs d'ordres 3/3, dernière) livrée localement. Route `api/admin/donneurs-ordres/recap/pdf` calquée sur le recap chauffeur (guard dirigeant+régulateur, isolation org, courses 1 requête + patients `.in()` patients_safe, audit log) + composant `RecapDonneurPdf` (charte PDF commune, total € = tarif stocké reflétant la grille B2B sans recalcul) + bouton « Récap PDF » fiche donneur (période pré-remplie selon modalite_facturation). À la demande ; récap automatique planifié tracé lot futur dépendant EMAIL_ENABLED. **Chaîne d'extensions B2B COMPLÈTE** (cœur 07.01 + grille 07.02 + groupée 07.03 + récap 07.04). **GATE 08.x toujours en attente** : test isolation 2-orgs data-cache sur preview. 86 phases feature livrées + 3 lots hors compte (06.40 hygiène, 06.56 méthode, 06.67 chore CI) (cf. commentaire de comptage dans le frontmatter). 4 release toggles OFF pré-infra : GEOLOC, MESSAGING, UPLOAD_DOCS, EMAIL. Restantes : lot alignement versions Supabase (débloque le typage des écritures, DEC-155) ; Lot 3 Suspense (différé) ; Phase 09 HDS (registre §1.1/§4.3) + Phase 10 géoloc réelle ; choix provider email (registre §1.2, débloque le récap B2B automatique + messagerie) ; messagerie complète (registre §1.4) ; contacts donneurs multiples (registre §6.4) + portail self-service V1.5 (§6.5) ; étapes restantes du plan d'audit.
...
Phase: 07.04 (récap PDF périodique B2B — extension donneurs d'ordres 3/3, dernière) livrée localement (2026-06-12). PR à ouvrir. Chaîne B2B complète.
Phase next: lot alignement versions Supabase (typage écritures, DEC-155) ; valider l'isolation 2-orgs data-cache sur preview (08.x) ; Lot 3 Suspense (différé) ; choix provider email (registre §1.2 — débloque le récap B2B automatique planifié + messagerie) ; messagerie complète (registre §1.4) ; contacts donneurs multiples (§6.4) ; portail self-service V1.5 (§6.5) ; Phase 09 HDS ; Phase 10 géoloc.
Status: 86 phases feature + 3 lots hors compte. Récap PDF B2B : route api/admin/donneurs-ordres/recap/pdf (guard dirigeant+régulateur, isolation org, courses 1 requête + patients .in() patients_safe, audit log) + composant RecapDonneurPdf (charte PDF commune, total € = tarif stocké reflétant la grille B2B sans recalcul) + bouton fiche donneur (période pré-remplie selon modalite_facturation). À la demande ; automatique planifié = lot futur dépendant EMAIL_ENABLED. Chaîne extensions B2B COMPLÈTE (07.01 cœur + 07.02 grille + 07.03 groupée + 07.04 récap). Data-cache référentiels 4/5 (gate preview en attente). 4 release toggles OFF pré-infra.
...
Last activity: Phase 07.04 (récap PDF périodique B2B, extension B2B 3/3, dernière). D-01 route api/admin/donneurs-ordres/recap/pdf calquée sur recap chauffeur (DEC-116) : params ordering_party/from/to, guard dirigeant+régulateur (403), isolation org (donneur via RLS → cross-org 404), courses 1 requête .eq('ordering_party_id') + patients .in() patients_safe (pas de N+1, RGPD), audit log avant rendu. D-02 composant RecapDonneurPdf réutilise PdfDocument/pdfStyles (charte commune) ; total € = tarif_amount_eur stocké (reflète grille B2B 07.02, pas de recalcul). D-03 bouton « Récap PDF » fiche donneur (édition), période pré-remplie selon modalite_facturation. D-04 à la demande ; automatique planifié tracé lot futur (EMAIL_ENABLED, registre §6.3). Garde-fous : charte PDF commune ; pas de N+1 ; dirigeant/régulateur + isolation org ; tarif non recalculé ; patients_safe. typecheck+lint(0 err, 8 warn)+build verts. 0 migration, 0 dépendance. Résout registre §6.3. Chaîne B2B complète. DEC-159 LOCKED. PR à ouvrir.
Précédent: 07.03 demande groupée B2B (DEC-158), 07.02 grille tarifaire B2B (DEC-157), 09.03 perf N+1 crons SMS (DEC-156).
