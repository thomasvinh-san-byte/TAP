...
# Dernière phase livrée : 12.01 (mode alerte météo / cyclone, T1). Dernier DEC : 170 (12.01). Dernier ADR : ADR-013 (06.33).
progress:
  total_phases: 98
  completed_phases: 96
  total_plans: 102
...
Phase: 12.01 (T1 — mode alerte météo / cyclone, cœur) livrée localement (2026-06-12). PR à ouvrir.
Phase next: extension replanification météo J+1/J+2 (réutilisera le socle 10.01, registre) ; trous V1 restants — T2 accompagnant patient, T3 incidents mineurs, T4 2FA dirigeant/régulateur ; générer les clés VAPID (secrets Vercel) pour activer le push en prod (06.69) ; rebrancher le détour réglementaire sur le routing réel quand disponible (registre §4.2, post-HDS) ; KPIs 5.20 restants (registre §9.4) ; lots suivants replanification (registre §8) ; lot alignement versions Supabase (typage écritures, DEC-155) ; valider l'isolation 2-orgs data-cache sur preview (08.x) ; Lot 3 Suspense (différé) ; choix provider email (registre §1.2) ; messagerie complète (registre §1.4) ; contacts donneurs multiples (§6.4) ; portail self-service V1.5 (§6.5) ; Phase 09 HDS ; Phase 10 géoloc.
Status: 96 phases feature + 4 lots hors compte. Mode alerte météo : table dédiée weather_alerts (un seul actif/org, index unique partiel, RLS forcée) ; statut annulee_meteo ; setWeatherAlertAction + bandeau cockpit ; cancelRidesBatchWeatherAction (jour/zone, compare-and-set validee/assignee) ; SMS annulation_meteo best-effort + push chauffeurs groupé. Route /meteo (régulateur). pgTAP 12. Replanif J+1/J+2 hors scope (tracée). Data-cache référentiels 5/6 (gate preview en attente). 4 release toggles OFF pré-infra.
...
Last activity: Phase 12.01 (T1 mode alerte météo / cyclone, cœur). Trou V1 critique (CdG l.380-385, US-REG-09) : alertes cyclone saisonnières La Réunion suspendant l'activité ; aucun état météo global. D-01 table dédiée weather_alerts (historique/audit, un seul actif/org via index unique partiel, RLS forcée lecture same-org / écriture dirigeant-régulateur / pas de DELETE). D-02 setWeatherAlertAction + bandeau cockpit. D-03 statut annulee_meteo (migration séparée additive). D-04 cancelRidesBatchWeatherAction({date, zone?}) compare-and-set (validee/assignee), bornes jour, ilike pickup_city, cancel_motif. D-05 SMS annulation_meteo best-effort post-commit (socle 09.03/10.02, consentement RGPD, trace). D-06 push chauffeurs groupé (06.69). D-07 replanif J+1/J+2 hors scope tracée. Route /meteo + nav « Météo ». Tests pgTAP weather_alerts_rls.sql (12). typecheck+lint(0 err)+build verts. 2 migrations, 0 dépendance. Résout T1 (cœur). DEC-170 LOCKED. PR à ouvrir.
Précédent: 11.01 conformité détour mutualisation décret 2025-202 (DEC-169), 09.04 fix concurrence anti-TOCTOU (DEC-168), 06.69 push PWA chauffeur (DEC-167).
