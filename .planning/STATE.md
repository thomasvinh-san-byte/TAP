...
status: executing
stopped_at: "Phase 07.07 (KPIs prescriptions — top prescripteurs + bons par statut) livrée localement. PR à ouvrir."
last_updated: "2026-06-13T02:00:00.000Z"
last_activity: Phase 07.07 (KPIs prescriptions — top prescripteurs + bons par statut ; 0 migration, 0 dépendance). Rend la chaîne prescriptions (07.05+07.06) exploitable pour le pilotage dirigeant (CdG §5.4/§5.20). Agrégation PURE LECTURE (résout registre §9.2/§9.3). D-01 bloc DashboardPrescriptions (queries-dashboard, modèle DashboardConformite) : bonsActifs / bonsProchesSeuil (actifs ≥80 % consommation, aligné alerte 80 % 07.06) / bonsEpuises / bonsExpires + topPrescripteurs[] (Top 5 par nombre de prescriptions). D-02 getPrescriptions : 1 select prescriptions (statut, prescriber_id, trajets) → comptes + top calculés en mémoire, libellés des 5 en 1 .in() sur prescribers (anti-N+1) ; ajouté au Promise.all existant du dashboard (parallélisé, 0 cascade), fallback gracieux. D-03 card prescriptions-card.tsx (modèle compliance-card) : section bons par statut (épuisés+expirés en évidence destructive, proches seuil en ambre) + section Top prescripteurs (5 avec nom + nb) ; rangée dédiée Prescriptions du tableau de bord. D-04 drill-down non construit (pas de page liste prescriptions filtrée par statut ; optionnel, non trivial). Garde-fous : agrégation seule (ne touche NI trigger NI statut 07.06) ; pas de N+1 ; RLS org ; RGPD (top = prescripteurs, pas patients) ; périmètre limité (PAS autres KPIs 5.20). Dette : prescriptions hors types.gen.ts → .from('prescriptions' as never) (DEC-155). typecheck+lint(0 err, 8 warn)+build verts. Suite registre §9.4 : KPIs 5.20 restants (top patients, top donneurs B2B, occupation, marge, impayés, litiges). DEC-164 LOCKED.
last_activity_prev: Phase 07.06 prescriptions/bons de transport (DEC-163). Phase 07.05 référentiel prescripteurs (DEC-162).
...
# + 07.07 KPIs prescriptions (top prescripteurs + bons par statut) = 91. (06.40
...
# Dernière phase livrée : 07.07 (KPIs prescriptions). Dernier DEC : 164 (07.07). Dernier ADR : ADR-013 (06.33).
progress:
  total_phases: 93
  completed_phases: 91
  total_plans: 96
...
**Current focus:** Phase 07.07 (KPIs prescriptions — top prescripteurs + bons par statut) livrée localement. Bloc `DashboardPrescriptions` au tableau de bord dirigeant (bons actifs/proches seuil 80 %/épuisés/expirés + Top 5 prescripteurs), agrégation pure lecture parallélisée (anti-N+1), card dédiée. Résout registre §9.2/§9.3 (CdG §5.4/§5.20). **GATE 08.x toujours en attente** : test isolation 2-orgs data-cache sur preview. 91 phases feature livrées + 3 lots hors compte (06.40 hygiène, 06.56 méthode, 06.67 chore CI) (cf. commentaire de comptage dans le frontmatter). 4 release toggles OFF pré-infra : GEOLOC, MESSAGING, UPLOAD_DOCS, EMAIL. Restantes : KPIs 5.20 restants (registre §9.4 — top patients, top donneurs B2B, occupation, marge, impayés, litiges) ; lots suivants replanification (registre §8) ; lot alignement versions Supabase (débloque le typage des écritures, DEC-155) ; Lot 3 Suspense (différé) ; Phase 09 HDS (registre §1.1/§4.3 — débloque le scan des bons + précision géoloc replanification) + Phase 10 géoloc réelle ; choix provider email (registre §1.2) ; messagerie complète (registre §1.4) ; contacts donneurs multiples (registre §6.4) + portail self-service V1.5 (§6.5) ; étapes restantes du plan d'audit.
...
Phase: 07.07 (KPIs prescriptions — top prescripteurs + bons par statut) livrée localement (2026-06-12). PR à ouvrir.
Phase next: KPIs 5.20 restants (registre §9.4 — top patients, top donneurs B2B, occupation, marge, impayés, litiges) ; lots suivants replanification (registre §8) ; lot alignement versions Supabase (typage écritures, DEC-155) ; valider l'isolation 2-orgs data-cache sur preview (08.x) ; Lot 3 Suspense (différé) ; choix provider email (registre §1.2) ; messagerie complète (registre §1.4) ; contacts donneurs multiples (§6.4) ; portail self-service V1.5 (§6.5) ; Phase 09 HDS ; Phase 10 géoloc.
Status: 91 phases feature + 3 lots hors compte. KPIs prescriptions : bloc DashboardPrescriptions (bonsActifs/bonsProchesSeuil ≥80 %/bonsEpuises/bonsExpires + Top 5 prescripteurs par nb de prescriptions) ; getPrescriptions = 1 select + libellés .in() (anti-N+1) ajouté au Promise.all dashboard ; card prescriptions-card (statuts en évidence + top) en rangée dédiée. Agrégation pure lecture (ne touche NI trigger NI statut 07.06). Résout registre §9.2/§9.3. 3 entités distinctes. Data-cache référentiels 5/6 (gate preview en attente). 4 release toggles OFF pré-infra.
...
Last activity: Phase 07.07 (KPIs prescriptions §5.4/§5.20). D-01 bloc DashboardPrescriptions (queries-dashboard) : bonsActifs/bonsProchesSeuil (≥80 %, aligné alerte 07.06)/bonsEpuises/bonsExpires + topPrescripteurs[] (Top 5 par nb de prescriptions). D-02 getPrescriptions : 1 select prescriptions → comptes + top en mémoire, libellés des 5 en 1 .in() sur prescribers (anti-N+1) ; ajouté au Promise.all existant (parallélisé) ; fallback gracieux. D-03 card prescriptions-card.tsx (modèle compliance-card) : bons par statut (épuisés+expirés destructive, proches seuil ambre) + Top prescripteurs ; rangée dédiée tableau de bord. D-04 drill-down non construit (optionnel). Garde-fous : agrégation seule (ne touche NI trigger NI statut 07.06) ; pas de N+1 ; RLS org ; RGPD (prescripteurs, pas patients) ; périmètre limité. Dette : prescriptions hors types.gen.ts → as never (DEC-155). typecheck+lint(0 err, 8 warn)+build verts. 0 migration, 0 dépendance. Suite registre §9.4. DEC-164 LOCKED. PR à ouvrir.
Précédent: 07.06 prescriptions/bons de transport (DEC-163), 07.05 référentiel prescripteurs (DEC-162), 10.02 SMS patients impactés réaffectation (DEC-161).
