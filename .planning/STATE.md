...
last_activity: 2026-05-18 — Hotfix UX patient form livré (PR #105) : label « Sexe » (FR médical conforme NIR/HAS/CNIL) + option X renommée « Non précisé » + autocomplete BAN adresse via wrapper PatientAddressField (réutilise composant courses partagé). Dette api-adresse.data.gouv.fr → Géoplateforme IGN inscrite CONCERNS Phase 06 (3 fichiers concernés, ancien endpoint déprécié fin janvier 2026). UI-PATTERNS enrichi 2 sections (terminologie médicale française + autocomplete adresse). Démo design partner cohérence terminologique + réduction erreurs saisie terrain. Précédent : Clôture Phase 04.7-bis FINALE après marathon ~4h custom domain (PR #98-#104 cumulées, custom domain tap-web-brown.vercel.app opérationnel triple fix, latences 686-977ms vs cible <300ms — Phase 04.7-perf-v2 différée).
progress:
  total_phases: 15
  completed_phases: 4
  total_plans: 24
