...
last_activity: 2026-05-18 — Hotfix CRITIQUE régression RLS récursion PR #101 livré : migration 20260518000001 restaure SECURITY DEFINER sur current_organization_id + current_user_role. Bénéfice perf PR #101 préservé (21 policies wrappées + 5 FK indexes intacts). Erreurs 500 BDD systématiques résolues (GET /profiles, /patients_safe). Démo design partner enfin opérationnelle après fix Vercel Project Settings Override toggles désactivés en parallèle. Cause root : migration 20260516000006 avait remplacé security definer par security invoker → récursion infinie RLS sur policy profiles_select_same_org → Postgres 500 → middleware Next.js interprète comme auth cassée → boucle redirect client. Pattern méta inscrit CONCERNS+UI-PATTERNS : SECURITY DEFINER obligatoire pour fonctions appelées dans policies RLS qui SELECT depuis tables RLS qu'elles policy-protègent. Précédent : Hotfix Vercel + Supabase URLs custom domain (vercel.json simplifié + Site URL Supabase aligné).
progress:
  total_phases: 15
  completed_phases: 4
  total_plans: 23
