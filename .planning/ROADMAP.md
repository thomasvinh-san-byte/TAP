...
- [ ] **Phase 06.17: Qualité de complétion des formulaires** — Aide à la saisie selon les normes UX/a11y (NN/G, Deque, Shopify, USWDS, W3C APG). Composant `<Field>` commun (label + helper text persistant + erreur, fin des Field locaux dupliqués) ; helper text d'exemple pour les champs à format ; numériques bornés en `inputMode=numeric` sans spinner. Composant `<Combobox>` éditable accessible (sur primitives existantes, sans nouvelle dépendance) ; Marque/Modèle en comboboxes dépendantes (modèle filtré par marque) sur catalogue `lib/vehicles/catalog.ts` ; saisie libre préservée + normalisation au submit. Pilote : formulaire véhicule ; extensible aux autres formulaires. 0 migration BDD, 0 dépendance npm.
