...
- [ ] **Phase 06.14: Migration tokens.json → Tailwind config** — Brancher le design system documenté en 06.13 sur le code, sans créer de divergence entre sources de vérité. Cible : `tokens.json` (DTCG) devient la seule source éditable → génération de `globals.css` (light + bloc `.dark`) via Style Dictionary. Corrige le pattern `hsl(var(--x) / <alpha-value>)` (opacité Tailwind cassée), migre les 2 fichiers à couleurs en dur, documente le workflow `tokens:build`. **Décisions dirigeant : dark généré depuis les tokens ; rester Tailwind v3 (v4 = décision séparée, couplage 06.9).** Recherche amont : `.planning/phases/06.14-migration-tokens-tailwind/06.14-RESEARCH.md`. Estimation indicative 5-8 h. Pas de migration BDD, pas de dépendance runtime.
