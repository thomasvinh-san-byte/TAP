...
- [ ] **Phase 06.9: Modernisation Next.js 15.5** — **Livré localement 2026-06-05.** Montée 14.2.35 → 15.5.19. Migration async complète des Request APIs (`params`/`searchParams`/`cookies`/`headers`) — codemod `next-async-request-api` + 84 sites `createClient()` Supabase serveur rebrandés `await createClient()` (async). Reprise manuelle : 2 casts `UnsafeUnwrapped*` retirés (interdits, D-02), 8 `ReturnType<typeof createClient>` → `Awaited<...>`, `resolveOrigin` async. `lib/geocoding/ban.ts` : cache explicite `{ cache: 'no-store' }` (D-04). `typedRoutes` activé (stable 15.5). Rewrite `/api/solver` mort purgé (orphelin Phase 06.12, ADR-010). `next-mdx-remote` downgradé 6→5 (peerDep React 19) + bascule `compileMDX` → `<MDXRemote>` + `force-dynamic` sur `/legal/*` (incident SSG résolu). React 18 conservé (R19 différé, ADR-007). ESLint au build conservé désactivé (nettoyage CI séparé, D-08). ADR-011 + DEC-095. 0 migration BDD.
