...
- [ ] **Phase 06.15: Refonte data tables (composant DataTable commun)** — Uniformiser les 13 data tables sur un composant `<DataTable>` sémantique unique (aujourd'hui chaque table réimplémente sa structure : 8 `<table>`, 4 `divide-y`, 1 mixte ; aucun composant partagé, ni `ui/table` shadcn). Consomme les tokens 06.14 + primitives EmptyState/Badge/Skeleton (06.11/06.13). API extensible (tri/pagination prévus) ; seul le tri accessible existant (caisse-table, `aria-sort` + URL params) généralisé en v1. Logique métier de chaque table préservée. RGAA (`<th scope>`, aria-sort, clavier), densité DEC-034, jour+nuit. **Option 3 dirigeant : uniformiser maintenant, enrichir tri/pagination plus tard.** 3 dropdowns de saisie exclus. Estimation 12-16 h. 0 migration BDD, 0 dépendance.
