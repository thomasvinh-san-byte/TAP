...
- [x] **Phase 06.10: Dettes techniques Phase 06.7** — Traitement des dettes D1 (hébergement Python solveur) et D2 (pipeline geocoding au moment création course) identifiées à la clôture Phase 06.7. **Livré 2026-06-03, 6 PR (Wave 1 hébergement Python : #207 expérience d'apprentissage, #208 bascule config legacy, #209 fix sys.path, #210 préfixe FastAPI conditionnel, #212 timeout client + maxDuration ; Wave 2 geocoding : #211 audit cas A déjà câblé Phase 04.7 + 3 tests Vitest scellement).** Résultat : chaîne Vercel Python OR-Tools techniquement fonctionnelle (`/api/solver/health` répond 200), mais walkthrough complet bloqué sur Vercel Hobby par plafond maxDuration 10s. Décision dirigeant 2026-06-03 : mock activé en prod+preview pour phase tests, réactivation vrai solveur reportée à Phase 06.11 candidate (cf. SUMMARY 06.10). D3 (passe UX) et D4 (audit casts) restent différées. Pipeline geocoding scellé par tests.
- [x] **Phase 06.11: Polish produit** — Tableau dirigeant enrichi (badges GAR conformité, alertes proactives, comparatif N vs N-1) + passe UX écran optimisation (modal d'ajustement collaboratif, visuels enrichis, couleurs distinctes par groupement, bouton re-calculer, skeleton loader) + finition démo (audit empty states transverses, showcase capture Phase 04-06). **3 waves, 23-33 h estimé. Aucune dépendance externe nouvelle. Side-quest D4-a opportuniste audit casts TypeScript inscrit en CONCERNS.md. DEC-084..087 LOCKED.** Sources d'inspiration capitalisées : HVI fleet 2026, Solvice OnRoute, RoadWarrior, étude tule2236, Stripe Balance (déjà adopté Phase 04.7), Doctolib (déjà adopté Phase 03.1).
- [x] **Phase 06.12: Solveur d'optimisation = heuristique TypeScript native** — **Livré 2026-06-04.** Décision (ADR-010, DEC-093) : abandon OR-Tools / Python / mock / hébergement séparé. Réécriture en heuristique TS native cluster-first/route-second dans `apps/web/src/lib/optimizer/` (`haversine.ts` + `solve-local.ts`). Justification : volume réel ≤ 500 courses absolu (contrat zod plafonne `rides.max(200)`), OR-Tools calibré 1000+ waypoints = disproportionné ; pour fenêtres temporelles petites + horaires quasi-fixes (dialyse), l'heuristique greedy est quasi-optimale ; indicateurs « estimés » DEC-081 → exactitude non contractuelle. Bénéfice central : supprime la SEULE barrière restante (hébergement Python), zéro coût marginal. Contrat `@tap/optimizer-client` (zod) inchangé → réversible. Supprimés : `apps/web/py/solver/` (13 fichiers), `_mock-solver.ts`, `client.ts` HTTP / `OptimizerError`, `vercel.json` Python builds, env vars `OPTIMIZER_USE_MOCK` / `OPTIMIZER_SERVICE_URL` (à retirer Vercel post-merge). 6 scénarios pytest portés en Vitest (13 tests verts). ADR-008 + ADR-009 marquées Superseded. 0 dépendance npm, 0 migration BDD.
- [x] **Phase 06.13: Foundations design system** — Phase documentaire pure (0 ligne code applicatif). **Livré 2026-06-03, PR #218** (4 livrables présents : `01-foundations.md` 319 l., `02-patterns-emergents.md` 398 l., `03-benchmark-foss.md` 159 l., `tokens.json` 276 l. validé `jq` ; DEC-088..091 LOCKED ; 0 ligne code applicatif confirmé). Cadre pour Phases 06.14 (migration tokens → Tailwind config), 06.15 (refonte data tables Carbon-inspired), 06.16 (refonte settings Linear+Stripe inspired).
- [x] **Phase 06.14: Migration tokens.json → Tailwind config** — **Livré 2026-06-04, PR #223.** `tokens.json` (DTCG) est désormais la source de vérité unique → génération via Style Dictionary v4 d'un fragment `tokens.generated.css` (CSS vars light + bloc `[data-theme='dark']`, noms de vars préservés en drop-in) + export JS `tokens.generated.ts` (hex) pour PDF + themeColor PWA. Architecture Token Sets (`tokens.json` base + `tokens.dark.json` override des 19 couleurs chromatiques). Pattern alpha `hsl(var(--x) / <alpha-value>)` corrigé sur 16 occurrences `tailwind.config.ts`. 4 vars latentes matérialisées (`--popover`, `--popover-foreground`, `--accent-foreground`, `--destructive-foreground` consommées sans token source). 3 sites hex migrés (2 PDF + themeColor) ; dérive `#1466DB` corrigée. Build câblé `tokens:build && next build`. Resté Tailwind v3 (DEC : v4 couplé à 06.9). 0 migration BDD, devDep seule.
- [x] **Phase 06.15: Refonte data tables (composant DataTable commun)** — **Livré 2026-06-04, PR #226.** Composant `<DataTable>` sémantique unique créé (`apps/web/src/components/data-table/`, consomme tokens 06.14 + primitives EmptyState/Badge/Skeleton) ; les 13 data tables uniformisées dessus (8 `<table>` + 4 `divide-y` + 1 mixte). `facture-apercu` converti en composant client (frontière RSC, D-07). Tri accessible de caisse-table généralisé (`aria-sort` + URL params, deep-link). API extensible (tri/pagination prévus, seul tri caisse implémenté v1). Logique métier de chaque table préservée (DEC-033). RGAA (`<th scope>`, aria-sort, clavier), densité DEC-034, jour+nuit. 3 dropdowns de saisie exclus. 0 migration BDD, 0 dépendance.
- [x] **Phase 06.16: PageHeader admin commun (refonte settings)** — **Livré 2026-06-04, PR #228.** Composant `<PageHeader>` (titre + description + slot actions, tokens 06.14, esprit Linear/Stripe) créé dans `apps/web/src/components/page-header/` et appliqué aux 16 pages admin (0 header manuel restant). Actions de `legal/registre` (ExportPdf + Nouvelle entrée) préservées dans le slot. Chrome globale (layout admin, nav, guard) inchangée. Strict : toolbar recherche/filtres différée. Présentation pure, 0 logique métier, 0 migration BDD, 0 dépendance.
- [x] **Phase 06.17: Conformité des champs de saisie (tous les champs)** — **Livré 2026-06-04, PR #230 + #231 + #232.** 132 champs du projet aux normes UX/a11y via composants communs : `<Field>` (helper text persistant lié `aria-describedby`, `name` explicite respecté), `<NumberField>` (inputMode numeric ou decimal, pas de spinner, défaut cohérent — règle `kind="counter"` démarre à `min`, `kind="optional"` reste vide avec hint d'unité), `<Combobox>` éditable accessible 100 % maison W3C APG (sur primitives existantes, DEC-003 préservée). Marque/Modèle = comboboxes dépendantes (catalogue `lib/vehicles/catalog.ts` 13 marques × ~5 modèles + normalize Title Case). 0 `type="number"` restant dans `apps/web/src`. `maxLength` posée sur champs à format (immat 9, NIR 19 avec espaces, tél 14, email 120, version 50, titre 200, mot de passe 128). Hints d'exemple posés sur dpia/dpa-prefill/dpo/accept-invite. Normalisation submit (trim partout, Title Case marque/modèle, casse canonique préservée sur sous-traitants). 23 tests Vitest (Field 5 + NumberField 8 + Combobox 8 + catalog 7). 3 PR : PR1 #230 composants communs + véhicule/chauffeur, PR2 #231 légal + tarifs + rattrapage défauts places, PR3 #232 reste + clôture. 0 migration BDD, 0 dépendance npm.
- [x] **Phase 06.18: Page de connexion — champs + UI aux normes** — **Livré 2026-06-04, PR #233.** `<PasswordInput>` commun (toggle Eye/EyeOff `type=button`, `aria-label` + `aria-pressed`, clavier) réutilisé login + accept-invite ; `inputMode="email"` ; pas d'autofocus (norme a11y). `<ThemeToggle>` extrait, posé dans le header AuthShell. AuthShell en Server Component. Cohérence dark mode (tokens 06.14). Reset MDP hors périmètre. 0 migration BDD, 0 dépendance.
- [x] **Phase 06.19: Branchement géocodage (récurrences + filet serveur)** — **Livré 2026-06-04, PR #236.** Récurrences géocodées : `AddressOrPOIPicker` dans les modales create/edit, coords persistées sur le template `ride_recurrences` (colonnes préexistantes, 0 migration) et propagées aux occurrences générées. Filet serveur `lib/geocoding/geocode-safety-net.ts` (idempotent, non bloquant si BAN down). Backfill étendu aux récurrences + occurrences. Section « courses exclues faute de coordonnées » dans l'optimisation (remplace le message trompeur). Alimente l'heuristique `solveLocal` (06.12). OSRM hors périmètre (DEC-056). 0 dépendance npm.
- [x] **Phase 06.9: Modernisation Next.js 15** — **Livré 2026-06-04, PR #238 (+ fix mdx #239).** Next 15.5, async APIs complètes, Serwist intact, purge rewrite solver. Correctif post-build : retour `next-mdx-remote` 6.0.0 (downgrade 06.9 injustifié corrigé — 6.x ne requiert pas React 19, devDep `react: ^18.2.0`, peerDep `>=16`, 5.0.0 vulnérable RCE), `pnpm.overrides` épinglent React 18.3.1 (dédup hoisting), `<MDXRemote>` rendu directement dans les pages (pattern recommandé Next 15), `turbo.json` `globalEnv` complété (POSTGRES_URL, APP_NIR_SEARCH_KEY, etc.). `force-dynamic` conservé sur `/legal/*` (bug Next 15 + SSG + mdx persiste indépendamment de la version, compromis documenté). ADR-011 + DEC-095. 0 migration BDD, 0 dépendance npm ajoutée.
- [~] **Phase 07: Mobile native chauffeur — ABANDONNÉE (décision dirigeant 2026-06-04, DEC-092)** — Mobile natif écarté : la PWA Phase 04.9 couvre le périmètre terrain retenu, le coût natif (10×, 25-40 h) n'est pas justifié au stade actuel. Réversible si business case mobile validé ultérieurement. Conservée pour traçabilité. _(Description d'origine ci-dessous, pour mémoire.)_ App native iOS + Android (React Native ou Capacitor) + géoloc continue + mode hors-ligne complet + reconnaissance vocale + push natives + mode lecture seule chauffeur. Phase 04.9 PWA peut suffire si business case mobile natif non validé (coût 10× inférieur, même périmètre fonctionnel). **Estimation : 25-40 h.**
...
- [ ] **Phase 10.0: Prototype géoloc / terrain (pré-HDS, données fictives)** — **Livré localement 2026-06-05.** Prépare le terrain géoloc (fonctionnel + UI/UX) sur données fictives, sans persister de vraie position (DEC-075). Capture **événementielle** aux pointages (pas de suivi continu — barrière PWA, RETEX). Cockpit affiche dernière position + âge (« vu il y a X min »), JAMAIS « live ». Composants livrés : table `driver_positions` + RLS + rétention 90j câblée, helper serveur `recordDriverPosition` gardé par flag `GEOLOC_ENABLED`, helper client `captureCurrentPosition` non bloquant (8s timeout, accuracy ≤ 100m), schémas zod partagés `driverPositionInputSchema`, route handlers `start`/`end`/`no-show` étendus, composant `<Map>` MapLibre + PMTiles (fallback OSM raster), hook `useDriverPositions` Realtime, panneau `DriverPositionsPanel` cockpit (carte + liste a11y + badge DÉMO), banner consentement chauffeur sur `/conduite`. Seed démo : 3 positions statiques sur les 3 chauffeurs démo (aucune animation, aucun simulateur). Capture premier-plan (D-04 watchPosition) différée. ADR-012 (MapLibre + PMTiles, 2 dépendances ajoutées). DEC-096 LOCKED. 1 migration BDD (`driver_positions`).
- [ ] **Phase 10: Géolocalisation opérationnelle temps réel** (ex-Phase 08) — Suivi position véhicules pour la régulation : carte cockpit live, ETA temps réel (régulateur + patient SMS), km à vide / km en charge réels, comparaison itinéraire prévu vs réalisé, historique 90 j puis purge (CDC §5.17). **Distincte de la géoloc certifiée facturation (hors périmètre, DEC-074).** Depends on: Phase 09 (HDS — DEC-075, position de véhicules-patients = donnée de santé indirecte). Contrainte technique : la capture GPS en arrière-plan n'est pas fiable en PWA (iOS surtout). Le mobile natif étant abandonné (Phase 07, DEC-092), le discuss 10 devra concevoir une solution PWA premier-plan dégradé (capture pendant l'usage actif chauffeur), pas de fallback natif. **Phase 10.0 (2026-06-05) a déjà câblé le socle évènementiel + UI cockpit ; Phase 10 = bascule réelle post-HDS via flag `GEOLOC_ENABLED`.** **Estimation : à cadrer en discuss.**
