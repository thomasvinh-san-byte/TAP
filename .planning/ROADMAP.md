...
- [ ] **Phase 06.19: Branchement géocodage (récurrences + filet serveur)** — Remplir les coordonnées des courses pour alimenter l'heuristique `solveLocal` (06.12). `AddressOrPOIPicker` dans le formulaire de récurrence + persistance des coords sur le template `ride_recurrences` (colonnes déjà existantes) + propagation aux occurrences générées. Filet serveur `geocodeBanSearch` si coords absentes (idempotent, non bloquant). Backfill étendu aux récurrences + propagation aux occurrences futures. Message d'exclusion optimisation reformulé (lien `/admin/maintenance`). OSRM hors périmètre (DEC-056). 0 migration BDD, 0 dépendance npm.
