...
- [x] **Phase 10.0: Prototype géoloc / terrain (pré-HDS, données fictives)** — **Livré 2026-06-04, PR #240.** Terrain géoloc préparé (fonctionnel + UI/UX) sans vraie position en prod. Carte MapLibre + extrait PMTiles 974 statique (ADR-012, fallback OSM raster). Table `driver_positions` + RLS + rétention 90j câblée. Capture événementielle au pointage (`capture-current-position`, NON bloquante : permission refusée/timeout/accuracy → pointage continue) ; capture premier-plan opportuniste (D-04 watchPosition) différée. Cockpit : panneau carte + marqueurs « vu il y a X min » (JAMAIS de faux live). Données démo statiques (pas de simulation animée). Flag `GEOLOC_ENABLED` OFF par défaut → 0 vraie position persistée pré-HDS. Capture événementielle et non continue : barrière technique PWA documentée (audit RETEX), pas un arbitrage de ressources. 0 dette RGPD. DEC-096.
