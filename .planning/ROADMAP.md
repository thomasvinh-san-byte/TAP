...
- [ ] **Phase 06.12: Réactivation solveur OR-Tools réel (candidate)** — Réactivation du vrai solveur après Phase 06.10 qui a montré que le mock suffit en bêta. 3 options cadrées dans `docs/dette-technique/2026-06-03-enquete-patterns-solveur-cout.md` : (A) bascule Vercel Pro + cron warm-up (~$20/mois, effort 1-2h), (B) refactor WASM via `reinterpretcat/vrp` Rust+WASM (effort 12-20h, gratuit), (C) container long-running Fly.io/Railway/Cloud Run (effort 4-8h, ~$5-20/mois). Sans urgence — le mock UX est fonctionnellement équivalent pour la phase tests. À déclencher si prospect demande qualité OR-Tools réelle.
