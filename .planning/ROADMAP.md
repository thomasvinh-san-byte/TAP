...
- [ ] **Phase 06.16: PageHeader admin commun (refonte settings)** — Extraire un composant `<PageHeader>` (titre + description + slot actions) consommant les tokens 06.14, esprit Linear/Stripe (sobre, dense), et l'appliquer aux 16 pages admin qui réimplémentent chacune leur `<header>` (aucun composant partagé). La chrome globale (layout admin, nav, guard) reste inchangée. **Strict (dirigeant) : cohérence des en-têtes ; toolbar recherche/filtres différée.** Présentation pure, 0 logique métier, 0 migration BDD, 0 dépendance. Estimation 5-8 h.
