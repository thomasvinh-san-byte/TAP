...
- [ ] **Phase 06.18: Page de connexion — champs + UI aux normes** — Application des normes auth/UI 2025-2026 (NN/G, muz.li 4 problèmes UX login, web.dev, UX Patterns, anti-autofocus a11y) à `/login` et `/accept-invite`. Composant `<PasswordInput>` partagé (toggle afficher/masquer, `aria-pressed`, accessible clavier, slot suffixe). `inputMode="email"` sur le champ email login. Composant `<ThemeToggle>` extrait de `UserMenu` via hook partagé `useTheme()` (DRY), posé dans le header form de `AuthShell` pour bascule jour/nuit avant connexion. `AuthShell` repassé en Server Component (le toggle est l'unique îlot client). Revue rendu dark complet (déjà tokenisé 06.14, contraste WCAG AA). Reset MDP **exclu** (acté). 0 migration BDD, 0 dépendance npm (icônes lucide Eye/EyeOff/Sun/Moon déjà disponibles).
