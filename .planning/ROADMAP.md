...
- [ ] **Phase 06.17: Conformité des champs de saisie (tous les champs)** — Mettre les 132 champs du projet aux normes UX/a11y via composants communs : `<Field>` (helper text persistant), `<NumberField>` ou variante de Field (inputMode numeric, pas de spinner, défaut cohérent avec min — fin des champs min=1 démarrant vides), `<Combobox>` éditable accessible (sur primitives existantes, sans dépendance). Marque/Modèle en comboboxes dépendantes (catalogue `lib/vehicles/catalog.ts`). Corrections : 7 champs `type=number`, états initiaux incohérents (places, compteurs), `maxLength` sur champs à format (immat/tél/NIR), hints d'exemple, normalisation au submit. Gabarit = `patient-form-fields`. Découpé en 3 PR séquentielles : PR1 (#230) composants communs + véhicule/chauffeur ; PR2 légal + tarifs ; PR3 reste. 0 migration BDD, 0 dépendance npm.
