...
- [ ] **Observabilité Sentry** — Stack figée DEC-003 mais jamais installé. Côté `apps/web` (server + client) avec source maps Vercel et sampling adapté preview/prod. Cible : visibilité runtime sur Server Actions, Route Handlers, RSC errors. **ROI : indispensable avant client payant.**
- [ ] **Tests RLS systématiques (pgTAP)** — 25 migrations métier, 0 test RLS automatisé à ce jour. Audit RLS Phase 06 a vérifié les 28 tables manuellement, mais sans suite de tests qui défende contre une régression future. Prérequis implicite à l'audit HDS. **ROI : prévient une fuite multi-tenant.**
- [ ] **Error boundaries par segment** — 1 seul `error.tsx` (root). Priorité : `(driver)/conduite` (PWA hors-ligne = écran blanc fatal sans capture), puis `(app)/cockpit` (régulatrice 8h/j). Capture l'erreur dans Sentry + UI dégradée propre. **ROI : empêche les écrans blancs en exploitation.**
- [ ] **(Moyenne) Audit complet pattern DEC-041 sur 38 Server Actions** — Pattern row count check `.select('id')` après UPDATE pour défendre RLS bypass silencieux. Audit Phase 06 a couvert un échantillon ; le passage en revue exhaustif des 38 Server Actions reste à faire. Faisable maintenant, durcit le terrain avant production commerciale.
- [ ] **(Moyenne) Tests unit logique métier** — `packages/pricing` (déjà 100 % branches), `packages/recurrence` (déjà 100 % branches) : maintenir. À étendre : `lib/optimizer/solve-local.ts` (heuristique TS native — 13 tests scénarios, mais branches non auditées), `lib/geocoding/geocode-safety-net.ts` (6 tests existants), `lib/recurrence/build-rides-payload.ts` (5 tests). Bonne couverture en place ; reste à fermer les angles morts mesurés via `pnpm test --coverage`.
